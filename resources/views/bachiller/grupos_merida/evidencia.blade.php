...
@section('breadcrumbs')
    ...
    <a href="{{ url('bachiller_grupo_uady/'.$grupo->id.'/evidencia') }}" class="breadcrumb">Agregar evidencia</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'bachiller.bachiller_grupo_uady.guardar_actualizar_evidencia', 'method' => 'POST']) !!}
        <div class="card ">
          <div class="card-content ">
            ...
            {{-- GENERAL BAR--}}
            <div id="general">
                {{-- <input type="hidden" name="bachiller_grupo_uady_id" value="{{$grupo->id}}"> --}}
                <div class="col s12 m6 l4" style="display: none">
                    {!! Form::label('bachiller_grupo_id', 'Grupo *', array('class' => '')); !!}
                    <select id="bachiller_grupo_id" class="browser-default validate select2" required name="bachiller_grupo_id" style="width: 100%;">
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        ...
                        <p><b>Materia: </b>{{$grupo->bachiller_materia->matNombre}}</p>
                        <p><b>Grado-Grupo: </b>{{$grupo->gpoGrado}}-{{$grupo->gpoClave}}</p>
                        <p><b>Docente: </b>{{$grupo->bachiller_empleado->empNombre}} {{$grupo->bachiller_empleado->empApellido1}} {{$grupo->bachiller_empleado->empApellido2}}</p>
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        {!! Form::label('bachiller_mes_evaluacion_id', 'Mes *', array('class' => '')); !!}
                        <select id="bachiller_mes_evaluacion_id" class="browser-default validate select2" required name="bachiller_mes_evaluacion_id" style="width: 100%;">
                            <option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>
                            @if ($Evidencias != "")
                                @foreach ($meses as $mes)
                                    <option value="{{$mes->id}}" {{ $Evidencias->bachiller_mes_evaluacion_id == $mes->id ? 'selected="selected"' : '' }}>{{$mes->mes}}</option>
                                @endforeach
                            @else
                                @foreach ($meses as $mes)
...
@include('bachiller.grupos.funcionesJS')
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function obtenerDatos(grupo_id,mes_id) {
            ...
            $.get(base_url+`/bachiller_calificacion/api/getEvidencias/${grupo_id}/${mes_id}`, function(res,sta) {
        ...
        obtenerDatos($("#bachiller_grupo_uady_id").val(),$("#bachiller_mes_evaluacion_id").val())
        $("#bachiller_mes_evaluacion_id").change( event => {
            obtenerDatos($("#bachiller_grupo_uady_id").val(),event.target.value)
        });
     });
  </script>
