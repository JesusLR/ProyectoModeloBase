...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#fecha_asistencia").change( event => {
            ...
            $.get(base_url+`/bachiller_inscritos/obtenerAlumnosPaseLista/${grupo_id}/${event.target.value}`,function(res,sta){
                var registros_guardados = res.bachiller_asistencia.length;
                ...
                if(registros_guardados > 0){
                    ...
                    const data = res.bachiller_asistencia;
                    
    
                            const tableData = data.map(function(element, item){              
                    ...
                    const dataNuevoRegistro = res.cursosPaseLista;
                            ...
                            const tableData = dataNuevoRegistro.map(function(element, item){              
        ...
        $.get(base_url+`/bachiller_inscritos/obtenerAlumnosPaseLista/${grupo_id}/${fecha_asistencia}`,function(res,sta){
            var registros_guardados = res.bachiller_asistencia.length;
            var alumnos_curso = res.cursosPaseLista.length;
