...
@section('breadcrumbs')
    ...
    <a href="{{route('bachiller.bachiller_curso.create')}}" class="breadcrumb">Agregar preinscripción</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['enctype' => 'multipart/form-data', 'onKeypress' => 'return disableEnterKey(event)','route' => 'bachiller.bachiller_curso.store', 'method' => 'POST']) !!}
        <div class="card ">
                    ...
                    <div class="col s12 m6 l8">
                        ...
                        @if (isset($candidato))
                            ...
                            @if ($candidato->perFoto)
                                    ...
                                    @if ($candidato->perFoto && !strpos($candidato->perFoto, '.pdf'))
                                        <img height="200px" src="{{url('/bachiller_curso_images/' . $candidato->perFoto) }}" alt="">
                                    @endif
                                    @if($candidato->perFoto && strpos($candidato->perFoto, '.pdf'))
                                        <label>Imagen</label>
                                        <embed src="/bachiller_curso_images/{{$candidato->perFoto}}"
...
<script type="text/javascript">
    $(document).ready(function() {
      ...
      $('#alumno_id').on('change', function() {
        ...
        $.ajax({
            type: 'GET',
            url: base_url+'/bachiller_alumno/ultimo_curso/'+alumno_id,
            dataType: 'json',
      ...
      $('#departamento_id').on('change', function() {
        ...
        if(departamento_id) {
            getEscuelas(departamento_id, 'escuela_id');
            getPeriodosCurso(departamento_id);
        }
      });
    ...
    function buscarAlumno()
      {
        ...
        $.ajax({
            type: "POST",
            url: base_url + `/bachiller_alumno/api/getMultipleAlumnosByFilter`,
            data: {
        ...
        .done(function(res) {
            ...
            if(res.restringido){
                ...
                if(res.listaNegra.perSexo == 'F'){
                    swal('Upss', `La alumna ${res.listaNegra.perApellido1} ${res.listaNegra.perApellido2} ${res.listaNegra.perNombre} cuenta con restricción. Motivo: ${res.listaNegra.lnRazon}`, 'warning');
                }else{
                    swal('Upss', `El alumno ${res.listaNegra.perApellido1} ${res.listaNegra.perApellido2} ${res.listaNegra.perNombre} cuenta con restricción. Motivo: ${res.listaNegra.lnRazon}`, 'warning');
                }
