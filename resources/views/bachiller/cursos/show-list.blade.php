...
@section('breadcrumbs')
    ...
    <a href="{{route('bachiller.bachiller_curso.index')}}" class="breadcrumb">Lista de preinscritos</a>
@endsection
...
<div id="table-datatables">
    <h4 class="header">PREINSCRITOS BACHILLER</h4>
    @php use App\Models\User; @endphp
    @if (User::permiso("curso") != "D" && User::permiso("curso") != "P")
        <a href="{{ route('bachiller.bachiller_curso.create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
    ...
    <div class="row">
        <div class="col s12">
            <table id="tbl-curso-bachiller" class="responsive-table display" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        <th>Año</th>
                        <th>Período</th>
...
@include('bachiller.cursos.modales.modaHistorialPagos-bachiller')
@include('bachiller.cursos.modales.modaBajaCurso-bachiller')
@include('bachiller.cursos.modales.modalBajaARegular')
@include('bachiller.cursos.modales.modalAlumnoDetalle-bachiller')
...
<script type="text/javascript">
    function modalHistorialPagos(curso_id) {
        ...
        $('.modal').modal();
        //MOSTRAR GRUPOS
        if ($.fn.DataTable.isDataTable("#tbl-historial-pagos-bachiller")) {
            $('#tbl-historial-pagos-bachiller').DataTable().clear().destroy();
        }

        $.fn.dataTable.ext.errMode = 'throw';
        $('#tbl-historial-pagos-bachiller').dataTable({
            "destroy": true,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/bachiller_curso/listHistorialPagos/" + curso_id,
                beforeSend: function () {
    ...
    function modalHistorialPagosAluClave(aluClave) {
        ...
        $('.modal').modal();
        //MOSTRAR GRUPOS
        if ($.fn.DataTable.isDataTable("#tbl-historial-pagos-alu-bachiller")) {
            $('#tbl-historial-pagos-alu-bachiller').DataTable().clear().destroy();
        }

        $.fn.dataTable.ext.errMode = 'throw';
        $('#tbl-historial-pagos-alu-bachiller').dataTable({
            "destroy": true,
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/bachiller_alumno/listHistorialPagosAluclave/" + aluClave,
                beforeSend: function () {
                    $('.preloader-modal').fadeIn(200, function() {
                        $(this).append('<div id="preloader-modal"></div>');
                    });
    ...
    $(document).on("click", ".btn-modal-historial-pagos-bachiller", function (e) {
        e.preventDefault()
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-preinscrito-detalle", function (e) {
            ...
            $.get(base_url+`/bachiller_curso/api/curso/${curso_id}`, function(res,sta) {
...
$(document).on('click', '.confirmar-baja-alumno', function (e) {
    ...
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: `${base_url}/bachiller_curso/api/curso/${curso_id}/verificar_materias_cargadas`,
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-baja-curso-bachiller", function (e) {
            e.preventDefault()
            var curso_id = $(this).data("curso-id");
            $(".modalCursoId").val(curso_id)

            $.get(base_url+`/bachiller_curso/api/curso/infoBaja/` + curso_id, function(res, sta) {
                console.log(res);
            ...
            $.get(base_url+`/bachiller_curso/conceptosBaja`, function(res,sta) {
                res.forEach(element => {
            ...
            $('#tbl-posibles-hermanos').dataTable({
                ...
                "ajax": {
                    "type" : "GET",
                    'url': "bachiller_curso/listPosiblesHermanos/" + curso_id,
                    beforeSend: function () {
...
<script type="text/javascript">
    $(document).ready(function() {
        $.fn.dataTable.ext.errMode = 'throw';
        var tableCurso = $('#tbl-curso-bachiller').dataTable({
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            "pageLength": 5,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url +"/bachiller_curso/list",
                beforeSend: function () {
                    $('.preloader').fadeIn(200,function(){$(this).append('<div id="preloader"></div>');});
                },
                complete: function () {
