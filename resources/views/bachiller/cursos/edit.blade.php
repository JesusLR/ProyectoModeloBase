...
@section('breadcrumbs')
    ...
    <a href="{{url('bachiller_curso')}}" class="breadcrumb">Lista de Preinscripción</a>
    <a href="{{url('bachiller_curso/'.$curso->id.'/edit')}}" class="breadcrumb">Editar preinscripción</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
        {{ Form::open(['enctype' => 'multipart/form-data', 'method'=>'PUT','route' => ['bachiller.bachiller_curso.update', $curso->id]]) }}
        <div class="card ">
                ...
                <div class="row">                   
                    <div class="col s12 m6 l8">
                        <div class="file-field input-field">
                            ...
                            <input value="" type="file" name="curBachillerFoto">
                            </div>
                        ...
                        @if ($curso->curBachillerFoto)
                            @if (file_exists(base_path('storage/app/public/bachiller/cursos/fotos/' . $periodo->perAnio.'-'.$periodo->perNumero . '/'. $nombreCarpeta. '/' . $curso->curBachillerFoto)))
                            <img style="width:200px;" src="{{url('/bachiller_curso_images/' . $curso->curBachillerFoto . '/' . $periodo->perAnio.'-'.$periodo->perNumero.'/'.$nombreCarpeta) }}" alt="">
                            @endif
                        @endif
                    </div>
                    <div class="col s12 m6 l4" style="visibility: hidden;">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#plan_id").change( event => {
            ...
            $("#materia_id").empty();
            $("#materia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $.get(base_url+`/bachiller_cgt/api/cgts/${event.target.value}/${periodo_id}`,function(res,sta){
                res.forEach(element => {
        ...
        $("#periodo_id").change( event => {
            ...
            $("#cgt_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $("#materia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $.get(base_url+`/bachiller_cgt/api/cgts/${plan_id}/${event.target.value}`,function(res,sta){
                res.forEach(element => {
                    $("#cgt_id").append(`<option value=${element.id}>${element.cgtGradoSemestre}-${element.cgtGrupo}-${element.cgtTurno}</option>`);
                });
            });
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#gpoSemestre").change( event => {
            ...
            $("#materia_id").empty();
            $("#materia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $.get(base_url+`/bachiller_materia/materias/${event.target.value}/${plan_id}`,function(res,sta){
                res.forEach(element => {
...
<script>
    ...
    $("#curTipoBeca").change(function(){
        if($('select[id=curTipoBeca]').val() == ""){
           $("#curPorcentajeBeca").val("");
           $("#curObservacionesBeca").val("");
        }
	});
</script>
