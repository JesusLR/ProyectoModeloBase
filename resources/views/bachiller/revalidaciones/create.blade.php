...
@section('breadcrumbs')
    ...
    <a href="{{route('bachiller.bachiller_revalidaciones.create')}}" class="breadcrumb">Agregar revalidación</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'bachiller.bachiller_revalidaciones.store', 'method' => 'POST']) !!}
        <div class="card ">
                ...
                <div class="row" id="oculta">
                    <div class="col s12 m6 l4">
                        <div class="input-field">
                          {!! Form::number('aluClave', old('aluClave'), array('id' => 'aluClave', 'class' => 'validate','required')) !!}
                          {!! Form::label('aluClave', 'Clave de pago *', array('class' => '')); !!}
                        </div>
                    </div>     
                    <div class="col s12 m6 l6">
                        <label for="nombreAlu">Nombre del alumno (solo lectura)</label>
                        <input type="text" value="{{old('nombreAlu')}}" id="nombreAlu" readonly>
                    </div>
                </div>
                ...
                <div class="row">   
                    <div class="col s12 m6 l4">
                        ...
                        <select id="tipoOficio" data-tipoOficio-id="{{old('tipoOficio')}}" class="browser-default validate select2" required name="tipoOficio"
                            style="width: 100%;">
                            <option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>                           
                            <option value="RV" {{ old('tipoOficio') == 'RV' ? 'selected' : '' }}>RV</option>
                            <option value="RC" {{ old('tipoOficio') == 'RC' ? 'selected' : '' }}>RC</option>
                        </select>
                    </div>
                    <div class="col s12 m6 l4">
                        {!! Form::label('fechaOficio', 'Fecha Oficio/Rec *', ['class' => '']); !!}                       
                        {!! Form::date('fechaOficio', old('fechaOficio'), array('id' => 'fechaOficio', 'class' =>'validate')) !!}
                    </div>
                    <div class="col s12 m6 l4">
...
<script type="text/javascript">
    ...
    $("#aluClave").keyup( event => {
      
        //${event.target.value}
        $.get(base_url+`/bachiller_revalidaciones/alumno/${event.target.value}`,function(res,sta){
                
            if(res.length > 0 ){
                res.forEach(element => {
                    $("#nombreAlu").val(`${element.perApellido1} ${element.perApellido2} ${element.perNombre}`);                    
                });
            }else{
                $("#nombreAlu").val("");   
                var len = event.target.value.length;

                $('input#aluClave').on('change', function(){
                    if(len >= 8){
                        swal('Upss...', 'No se encontro alumno', 'info');  
                    }
                });
    ...
    $("#tipoOficio").change(function(){
        if($('select[id=tipoOficio]').val() == "RC"){
            $("#opcion").change( event => {        
                
                var plan_id = $("#plan_id").val();
                
                $.get(base_url+`/bachiller_revalidaciones_materias_rc/${event.target.value}/${plan_id}`,function(res,sta){
