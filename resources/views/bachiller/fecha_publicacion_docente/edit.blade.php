...
@section('breadcrumbs')
    ...
    <a href="{{route('bachiller.bachiller_fecha_publicacion_calificacion_docente.index')}}" class="breadcrumb">Lista fechas de captura calificaciones</a>
    <a href="{{url('bachiller_fecha_publicacion_calificacion_docente/'.$bachiller_calendario_calificaciones_docentes->id.'/edit')}}" class="breadcrumb">Editar fechas de captura calificaciones</a>
@endsection

@section('content')
<div class="row">
    <div class="col s12 ">
        {{ Form::open(['enctype' => 'multipart/form-data', 'method'=>'PUT','route' => ['bachiller.bachiller_fecha_publicacion_calificacion_docente.update', $bachiller_calendario_calificaciones_docentes->id]]) }}
        <div class="card ">
                ...
                <div id="general">
                        ...
                        <div class="col s12 m6 l4">
                            ...
                            <select id="bachiller_mes_evaluaciones_id" data-plan-id="{{old('bachiller_mes_evaluaciones_id')}}" class="browser-default validate select2" required name="bachiller_mes_evaluaciones_id"
                                ...
                                @foreach ($bachiller_mes_evaluaciones as $mes_evaluacion)
                                    <option value="{{$mes_evaluacion->id}}" {{ $mes_evaluacion->id == $bachiller_calendario_calificaciones_docentes->bachiller_mes_evaluaciones_id ? 'selected' : '' }}>{{$mes_evaluacion->mes}}</option>
                                @endforeach
                            </select>
                        </div>                
                    </div>
                    ...
                    <div class="row">
                        <div class="col s12 m6 l3">
                            ...
                            {!! Form::date('calInicioCaptura', \Carbon\Carbon::parse($bachiller_calendario_calificaciones_docentes->calInicioCaptura)->format('Y-m-d'), array('id' => 'calInicioCaptura', 'class' => 'validate')) !!}
                        </div>

                        <div class="col s12 m6 l3">
                            {!! Form::label('calFinCaptura', 'Fecha final de captura *', ['class' => '']); !!}
                            {!! Form::date('calFinCaptura', \Carbon\Carbon::parse($bachiller_calendario_calificaciones_docentes->calFinCaptura)->format('Y-m-d'), array('id' => 'calFinCaptura', 'class' => 'validate')) !!}
                        </div>

                        <div class="col s12 m6 l3">
                            {!! Form::label('calInicioRevision', 'Fecha inicio de revisión *', ['class' => '']); !!}
                            {!! Form::date('calInicioRevision', \Carbon\Carbon::parse($bachiller_calendario_calificaciones_docentes->calInicioRevision)->format('Y-m-d'), array('id' => 'calInicioRevision', 'class' => 'validate')) !!}
                        </div>

                        <div class="col s12 m6 l3">
                            {!! Form::label('calFinRevision', 'Fecha fin de revisión *', ['class' => '']); !!}
                            {!! Form::date('calFinRevision', \Carbon\Carbon::parse($bachiller_calendario_calificaciones_docentes->calFinRevision)->format('Y-m-d'), array('id' => 'calFinRevision', 'class' => 'validate')) !!}
                        </div>
                    </div>
