...
@section('breadcrumbs')
    ...
    <a href="{{ route('bachiller.bachiller_grupo_seq.create') }}" class="breadcrumb">Agregar Grupo</a>
@endsection

@section('content')

@php
    $perActualUser = DB::select("SELECT perActual FROM departamentos WHERE id=1");
    $collect = collect($perActualUser);
    $perActual = $collect[0]->perActual;

@endphp
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'bachiller.bachiller_grupo_seq.store', 'method' => 'POST']) !!}
        <div class="card ">
          <div class="card-content ">
            ...
            <div id="general">
                ...
                <div class="row">
                   <div class="col s12 m6 l4">
                        <label for="gpoRecuperacion" style="color: red">¿Es grupo de recuperación?</label>
                        <select id="gpoRecuperacion" class="browser-default validate select2" required name="gpoRecuperacion" style="width: 100%;">
                            <option value="0" {{old('gpoRecuperacion') == 0 ? "selected": ""}}>NO</option>
                            <option value="1" {{old('gpoRecuperacion') == 1 ? "selected": ""}}>SI</option>
                        </select>
                    </div>

                    <div class="col s12 m6 l4">
                        {!! Form::label('gpoSemestre2', 'Semestre *', array('class' => '')); !!}
                        <input class="gpoSemestre2Old" type="hidden" data-gpoSemestre2-id="{{old('gpoSemestre2')}}">
        ...
        $("#gpoRecuperacion").change(event => {
            ...
            if(event.target.value == 0){
                ...
                var semestreOld = $("#gpoSemestre2").data("gpoSemestre2-id")
                    ...
                    $("#gpoSemestre2").append(`<option value='${i}' ${selected}>${i}</option>`);
                }
            }
        });

        if($('select[id=gpoRecuperacion]').val() == 0){
            $("#periodo_id").change(event => {
        ...
        function postGuardarGrupoBachiller () {
            $.ajax({
                data: {
                    ...
                    "gpoRecuperacion": $("#gpoRecuperacion").val()

                },
                type: "POST",
                dataType: "JSON",
                url: base_url + "/bachiller_grupo_seq",
            })
            .done(function( data, textStatus, jqXHR ) {
                ...
                if (!data.res) {
                    ...
                    if (data.existeGrupo) {
                        if(data.msg.gpoMatComplementaria == ""){
    ...
    $("input[name=gpoACD]").change(function(){
        if ($(this).is(':checked') ) {
            ...
            $("#materia_id").change( event => {
                ...
                $.get(base_url+`/bachiller_grupo_seq/materiaComplementaria/${event.target.value}/${plan_id}/${periodo_id}/${gpoSemestre2}`,function(resACD,sta){
            ...
            $("#periodo_id").change( event => {
                ...
                $("#materia_acd_id").empty();
                $("#materia_acd_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/bachiller_grupo_seq/materiaComplementaria/${materia_id}/${plan_id}/${event.target.value}/${gpoSemestre2}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#materia_acd_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            $("#plan_id").change( event => {
                ...
                $("#materia_acd_id").empty();
                $("#materia_acd_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/bachiller_grupo_seq/materiaComplementaria/${materia_id}/${event.target.value}/${periodo_id}/${gpoSemestre2}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#materia_acd_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
