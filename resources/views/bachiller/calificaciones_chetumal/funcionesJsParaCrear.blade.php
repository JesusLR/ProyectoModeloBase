<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        function obtenerGrupos(grupoId) {
            ...
            $.get(base_url+`/bachiller_calificacion_seq/api/getGrupos/${grupoId}`, function(res,sta) {
                ...
                res.forEach(element => {
                    ...
                    $("#grupo_id2").append(`<option value=${element.bachiller_grupo_id} ${selected}>${element.gpoGrado}${element.gpoClave}, Programa: ${element.progNombre}, Clave materia: ${element.matClave}</option>`);
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function obtenerMaterias2(materiaId2) {
            ...
            $.get(base_url+`/bachiller_calificacion_seq/api/getMaterias2/${materiaId2}`, function(res,sta) {

                //seleccionar el post preservado
                var materia2SeleccionadoOld = $("#materia_id2").data("materia_id2")
                $("#materia_id2").empty()
                res.forEach(element => {
                    var selected = "";
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function obtenerAlumnos(alumnoId) {
            $.get(base_url+`/bachiller_calificacion_seq/api/getAlumnos/${alumnoId}`, function(res,sta) {
                    ...
                    const data = res;
                        const tableData = data.map(function(element){
                            ...
                            return (
                                `<tr>
                                    <td><input name='bachiller_inscrito_id[]' type='hidden' value='${element.id}'></td>
    ...
    $(document).ready(function() {
        ...
        function obtenerMesEvaluacion(mes_id) {
            ...
            $.get(base_url+`/bachiller_grupo_seq/getMesEvidencias/${mes_id}`, function(res,sta) {
                ...
                var mesEvi = res.mesEvidencia
                //seleccionar el post preservado
                var numeroEvaSeleccionadoOld = $("#bachiller_grupo_evidencia_id").data("mes-idold")
                $("#bachiller_grupo_evidencia_id").empty()

                if(res != ""){
                    $("#bachiller_grupo_evidencia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                    mesEvi.forEach(elemento => {

                        var selected = "";
                        if (elemento.id === numeroEvaSeleccionadoOld) {
                            console.log("entra")
                            console.log(elemento.id)
                            selected = "selected";
                        }
      
                        $("#bachiller_grupo_evidencia_id").append(`<option value=${elemento.id} ${selected}>${elemento.mes}</option>`);
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        obtenerNumEvaluacion($("#bachiller_grupo_evidencia_id").val())
        $("#bachiller_grupo_evidencia_id").change( event => {
