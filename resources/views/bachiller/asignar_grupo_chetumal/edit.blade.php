...
@section('breadcrumbs')
    ...
    <a href="{{route('bachiller.bachiller_asignar_grupo_seq.index')}}" class="breadcrumb">Lista de Inscritos Materia</a>
    <a href="{{url('bachiller_asignar_grupo_seq/'.$inscrito->id.'/edit')}}" class="breadcrumb">Editar inscrito materia</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
        {{ Form::open(array('method'=>'PUT','route' => ['bachiller.bachiller_asignar_grupo_seq.update', $inscrito->id])) }}
        <div class="card ">
                    ...
                    <div class="col s12 ">
                        ...
                        <select id="grupo_id" class="browser-default validate select2" required name="grupo_id" style="width: 100%;">
                            ...
                            @foreach($grupos as $grupo)
                                <option value="{{$grupo->id}}" @if($inscrito->bachiller_grupo_id == $grupo->id) {{ 'selected' }} @endif>{{"Grupo: ".$grupo->gpoGrado.'-'.$grupo->gpoClave.'-'.$grupo->gpoTurno.' '.$grupo->gpoMatComplementaria. ", Materia: ".$grupo->bachiller_materia->matClave.'-'.$grupo->bachiller_materia->matNombre.' '."Maestro: ".$grupo->bachiller_empleado->empNombre.' '.$grupo->bachiller_empleado->empApellido1.' '.$grupo->bachiller_empleado->empApellido2}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#curso_id").change( event => {
            ...
            $.get(base_url+`/bachiller_asignar_grupo_seq/grupos/${event.target.value}`, function(res, sta) {

                res.forEach(element => {
                    if (!element.optNombre) {
                        $("#grupo_id").append(`<option value=${element.id}>
                            Grupo: ${element.gpoGrado}-${element.gpoClave}-${element.gpoTurno} ${element.gpoMatComplementaria},
                            Materia: ${element.matClave}-${element.matNombre}
                            Maestro: ${element.empNombre} ${element.empApellido1} ${element.empApellido2}
                        </option>`);
                    }
