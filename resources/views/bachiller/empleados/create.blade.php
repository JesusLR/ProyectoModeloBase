...
@section('breadcrumbs')
    ...
    <a href="{{url('bachiller_empleado/create')}}" class="breadcrumb">Agregar empleado</a>
@endsection
...
<div class="row">
  <div class="col s12 ">
    {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'bachiller.bachiller_empleado.store', 'method' => 'POST']) !!}
      <div class="card ">
 ...
 @include('bachiller.scripts.funcionesAuxiliares')
    ...
    <script>
        ...
        function verificarPersona() {
            $.ajax({
                type:'GET',
                url: base_url + '/bachiller_empleado/verificar_persona',
                data: $('form').serialize(),
                //dataType: 'json',
        ...
        function reactivarEmpleado(empleado_id) {
            $.ajax({
                type:'POST',
                url: base_url+'/bachiller_empleado/reactivar_empleado/'+empleado_id,
                data:{empleado_id: empleado_id, '_token':'{{csrf_token()}}'},
                dataType:'json',
                success: function(empleado) {
                    window.location = base_url+'/bachiller_empleado/'+empleado.id+'/edit';
                },
                error: function(jqXhr, textStatus, errorMessage) {
                    console.log(errorMessage);
                }
        ...
        function alumno_crearEmpleado(alumno_id) {
            $.ajax({
                type:'POST',
                url: base_url+'/bachiller_empleado/registrar_alumno/'+alumno_id,
                data: $('form').serialize(),
                dataType:'json',
                success: function (empleado) {
                    window.location = base_url+'/bachiller_empleado/'+empleado.id+'/edit';
