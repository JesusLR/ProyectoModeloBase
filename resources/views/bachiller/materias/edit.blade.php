...
<div class="row">
    <div class="col s12 ">
        {{ Form::open(array('method'=>'PUT','route' => ['bachiller.bachiller_materia.update', $materia->id])) }}
        <div class="card ">
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        <div class="input-field">
                            {!! Form::text('matClave', $materia->matClave, array('id' => 'matClave', 'class' => 'validate', 'required', 'maxlength'=>'30')) !!}
                            {!! Form::hidden('matClaveAnterior', $materia->matClave, array('id' => 'matClaveAnterior', 'class' => 'validate', 'required', 'maxlength'=>'30')) !!}
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        ...
                        <select id="matVigentePlanPeriodoActual" class="browser-default validate select2" name="matVigentePlanPeriodoActual" style="width: 100%;">
                            ...
                            <option value="SI" {{ $materia->matVigentePlanPeriodoActual == 'SI' ? 'selected' : '' }}>SI</option>
                            <option value="NO" {{ $materia->matVigentePlanPeriodoActual == 'NO' ? 'selected' : '' }}>NO</option>
                        </select>
                    </div>                    
                </div>
                <div class="row">
                    ...
                    <div class="col s12 m6 l4">
                        ...
                        <select id="matClasificacion" class="browser-default validate select2" name="matClasificacion" style="width: 100%;">
                            ...
                            <option value="U" {{$materia->matClasificacion == "U" ? "selected": ""}}>OCUPACIONAL</option>
                            ...
                            <option value="B" {{$materia->matClasificacion == "B" ? "selected": ""}}>BÁSICA INGLES</option>
                            @endif
                            
                            @if (auth()->user()->campus_cch == 1)
                            <option value="B" {{$materia->matClasificacion == "B" ? "selected": ""}}>BÁSICA</option>
                            <option value="X" {{$materia->matClasificacion == "X" ? "selected": ""}}>EXTRACURRICULAR</option>
                            <option value="C" {{$materia->matClasificacion == "C" ? "selected": ""}}>COMPETENCIAS</option>
                            <option value="E" {{$materia->matClasificacion == "E" ? "selected": ""}}>ESPECIALIDAD</option>
                            @endif
                        </select>
                        <input type="hidden" name="valorSeleccionado" id="valorSeleccionado" value="{{$materia->matTipoGrupoMateria}}">
...
<script>
    $(document).ready(function(){
        ...
        $("#valorSeleccionado").val($('select[name="matClasificacion"] option:selected').text());
        
        $("#matClasificacion").change(function(){

            var asignarValor = $('select[name="matClasificacion"] option:selected').text();
            if(asignarValor == "BÁSICA"){
                asignarValor = "BASICA";
            }

            if(asignarValor == "BÁSICA INGLES"){
                asignarValor = "BASICA INGLES";
            }

            if(asignarValor == "EXTRAOFICIAL"){
                asignarValor = "EXTRA";
            }
            $("#valorSeleccionado").val(asignarValor);
        });
    });
</script>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#departamento_id").change( event => {
            ...
            $.get(base_url+`/bachiller_periodo/api/periodos/${event.target.value}`,function(res2,sta){
                var perSeleccionado;
...
$(document).on('click', '#agregarPrograma', function (e) {
    ...
    if(programa_id != "" && programa_id != null){
        if(recorrerProgramas(programa_id)){
            $.get(base_url+`/bachiller_programa/api/programa/${programa_id}`,function(res,sta){
