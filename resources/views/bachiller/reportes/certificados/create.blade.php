...
@section('breadcrumbs')
    ...
    <a href="" class="breadcrumb">Lista de certificados pagados</a>
@endsection

@section('content')
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'bachiller.bachiller_certificados_pagados.imprimir', 'method' => 'POST', 'target' => '_blank']) !!}
        <div class="card ">
          <div class="card-content ">
            <span class="card-title">LISTA DE CERTIFICADOS PAGADOS</span>
            ...
            <div id="filtros">
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        {!! Form::label('estatus_certificado', 'Alumnos (Certificados)', ['class' => '']); !!}
                        <select name="estatus_certificado" id="estatus_certificado" class="browser-default validate select2" style="width: 100%;">
                            @foreach($estatus_certificado as $key => $value)
                                <option value="{{$key}}" @if(old('estatus_certificado') == $key) {{ 'selected' }} @endif>
                                    {{$value}}
                                    @if ($key == "R+P")
                    ...
                    <div class="col s12 m6 l4">
                        ...
                        <select name="formato" id="formato" class="browser-default validate select2" style="width: 100%;">
                            @foreach($formato as $key => $value)
                                <option value="{{$key}}" @if(old('formato') == $key) {{ 'selected' }} @endif>
                                    {{$value}}
                                    @if ($key == "R+P")
                                        (inscritos y preinscritos)
                                    @endif
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        <div class="input-field col s12 m6 l6" style="display: none;">
                            {!! Form::number('cgtGradoSemestre', 6, array('id' => 'cgtGradoSemestre', 'class' => 'validate','min'=>'0')) !!}
