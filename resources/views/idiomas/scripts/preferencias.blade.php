<script type="text/javascript">
    $(document).ready(function() {
        ...
        $.get(base_url+`/idiomas_api/departamentos/${ubicacion_id}`,function(res,sta){
            var perActual;
            res.forEach(element => {
                var selected = "";
            ...
            $.get(base_url+`/idiomas_periodo/api/periodos/${departamento_id}`,function(res2,sta){
                var perSeleccionado;
            ...
            $.get(base_url+`/idiomas_api/escuelas/${departamento_id}`,function(res,sta){
                res.forEach(elementEsc => {
                    var selected = "";
                    if(elementEsc.id == escuela_id){
                        selected = "selected";
                    }
                    $("#escuela_id").append(`<option value=${elementEsc.id} ${selected}>${elementEsc.escClave}-${elementEsc.escNombre}</option>`);
                });
                //OBTENER PROGRAMAS CON ESCUELA
                $.get(base_url+`/idiomas_programa/api/programas/${escuela_id}`,function(res,sta){
                    res.forEach(element => {
                    ...
                    if(programa_id != "" && programa_id != null){
                        ...
                        $.get(base_url+`/idiomas_plan/api/planes/${programa_id}`,function(res2,sta){
                            res2.forEach(element => {
                                ...
                                if(periodo_id != "" && periodo_id != null){
                                    ...
                                    $.get(base_url+`/idiomas_cgt/api/cgts/${plan_id}/${periodo_id}`,function(res3,sta){
                            ...
                            if(plan_id != "" && plan_id != null){
                                ...
                                $.get(base_url+`/idiomas_plan/plan/semestre/${plan_id}`,function(res,sta){
                                ...
                                if(cgt_id != "" && cgt_id != null){
                                    ...
                                    $.get(base_url+`/idiomas_materia/materias/${cgt_id}`,function(res4,sta){
