...
@section('breadcrumbs')
    ...
    <a href="{{route('curso_idiomas.index')}}" class="breadcrumb">Lista de preinscritos</a>
@endsection
...
<div id="table-datatables">
    <h4 class="header">PREINSCRITOS IDIOMAS</h4>
    <a href="{{ route('curso_idiomas.create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
        <i class="material-icons left">add</i>
    </a>
    <br><br>
    <div class="row">
        <div class="col s12">
            <table id="tbl-curso-idiomas" class="responsive-table display" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        <th>Período</th>
                        <th>Año</th>
...
@include('idiomas.curso_preinscrito.modales.modalPreinscritoDetalle')
@include('idiomas.curso_preinscrito.modales.modaHistorialPagos-idiomas')
@include('idiomas.curso_preinscrito.modales.modaBajaCurso-idiomas')
@include('idiomas.curso_preinscrito.modales.modalBajaARegular')
    ...
    $(document).on("click", ".btn-modal-ficha-pago", function(e) {
        ...
        if(pedirConfirmacion == 'SI') {
            swal({
                ...
                if (isConfirm) {
                    window.open("idiomas_curso/crearReferencia/" + curso_id + "/" + "si", "_blank");
                } else {
                    window.open("idiomas_curso/crearReferencia/" + curso_id + "/" + "no", "_blank");
                }
                swal.close()
            });
        } else {
    ...
    $(document).on("click", ".btn-modal-ficha-pago-hsbc", function(e) {
        ...
        if(pedirConfirmacion == 'SI') {
            swal({
                ...
                if (isConfirm) {
                    window.open("idiomas_curso/crearReferenciaHSBC/" + curso_id + "/" + "si", "_blank");
                } else {
                    window.open("idiomas_curso/crearReferenciaHSBC/" + curso_id + "/" + "no", "_blank");
                }
                swal.close()
            });
        } else {
...
<script type="text/javascript">
    function modalHistorialPagos(curso_id) {
        ...
        $('.modal').modal();
        //MOSTRAR GRUPOS
        if ($.fn.DataTable.isDataTable("#tbl-historial-pagos-idiomas")) {
            $('#tbl-historial-pagos-idiomas').DataTable().clear().destroy();
        }

        $.fn.dataTable.ext.errMode = 'throw';
        $('#tbl-historial-pagos-idiomas').dataTable({
            "destroy": true,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/api/idiomas_curso/listHistorialPagos/" + curso_id,
                beforeSend: function () {
    ...
    function modalHistorialPagosAluClave(aluClave) {
        ...
        $('.modal').modal();
        //MOSTRAR GRUPOS
        if ($.fn.DataTable.isDataTable("#tbl-historial-pagos-alu-idiomas")) {
            $('#tbl-historial-pagos-alu-idiomas').DataTable().clear().destroy();
        }

        $.fn.dataTable.ext.errMode = 'throw';
        $('#tbl-historial-pagos-alu-idiomas').dataTable({
            "destroy": true,
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/idiomas_alumnos/listHistorialPagosAluclave/" + aluClave,
                beforeSend: function () {
                    $('.preloader-modal').fadeIn(200, function() {
                        $(this).append('<div id="preloader-modal"></div>');
                    });
    ...
    $(document).on("click", ".btn-modal-historial-pagos-idiomas", function (e) {
        e.preventDefault()
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-preinscrito-detalle", function (e) {
            ...
            $.get(base_url+`/api/curso/${curso_id}`, function(res,sta) {
                ...
                if (res.curso.curExaniFoto && !res.curso.curExaniFoto.includes(".pdf")) {
...
$(document).on('click', '.confirmar-baja-alumno', function (e) {
    ...
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: `${base_url}/api/idiomas_curso/${curso_id}/verificar_materias_cargadas`,
        data: {'curso_id': curso_id},
        success: function(data) {
            if(data.tiene_materias_cargadas) {
                let mensajeAdicional = "El alumno tiene materias cargadas a este curso.";
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-baja-curso", function (e) {
            ...
            $.get(base_url+`/api/idiomas_curso/infoBaja/` + curso_id, function(res, sta) {
                console.log(res);
            ...
            $.get(base_url+`/api/idiomas_curso/conceptosBaja`, function(res,sta) {
                res.forEach(element => {
            ...
            $('#tbl-posibles-hermanos').dataTable({
                ...
                "ajax": {
                    "type" : "GET",
                    'url': "api/idiomas_curso/listPosiblesHermanos/" + curso_id,
                    beforeSend: function () {
...
<script type="text/javascript">
    $(document).ready(function() {
        $.fn.dataTable.ext.errMode = 'throw';
        var tableCurso = $('#tbl-curso-idiomas').dataTable({
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            "pageLength": 5,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url +"/idiomas_curso/list",
                beforeSend: function () {
                    $('.preloader').fadeIn(200,function(){$(this).append('<div id="preloader"></div>');});
                },
                complete: function () {
            ...
            "columns":[
                ...
                {data: "aluEstado",name:"alumnos.aluEstado"},
                ...
                {data: "curEstado"},
                {data: "curFechaBaja"},
                {data: "gpoGrado",name:"idiomas_grupos.gpoGrado"},
                {data: "gpoClave",name:"idiomas_grupos.gpoClave"},
                // {data: "beca"},
