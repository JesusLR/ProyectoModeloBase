...
function getDepartamentosListaCompleta(ubicacion_id, targetSelect = 'departamento_id', val = null, dataName = null) {
    ...
    $.ajax({
        type: 'GET',
        url: base_url + '/preescolar_curso/getDepartamentosListaCompleta/' + ubicacion_id,
        dataType: 'json',
        data: { ubicacion_id: ubicacion_id },
        success: function(departamentos) {
            if (departamentos) {
...
function getDepartamentos_claves(ubicacion_id, targetSelect, val = null, dataName = null) {
    ...
    $.ajax({
        type: 'GET',
        url: base_url + '/preescolar_grupo/api/departamentos/' + ubicacion_id,
        dataType: 'json',
        data: { ubicacion_id: ubicacion_id },
        success: function(departamentos) {
            if (departamentos) {
...
function obtenerEscuelas (departamentoId) {
    ...
    $.get(base_url+`/idiomas_curso/api/escuelas/${departamentoId}`,function(res,sta){
...
function getMaterias(plan_id, targetSelect = 'materia_id', val = null, dataName = null) {
    ...
    $.ajax({
        type: 'GET',
        url: base_url + '/preescolar_curso/getMateriasByPlan/' + plan_id,
        dataType: 'json',
        data: { plan_id: plan_id },
...
function getGrupos_plan_periodo(plan_id, periodo_id, targetSelect, val = null, dataName = null) {
    var select = $('#' + targetSelect);
    current_data = dataName || 'cgt-id';
    current_value = val || select.data(current_data);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
        url: base_url + '/api/idiomas_grupos/' + plan_id + '/' + periodo_id,
        dataType: 'json',
        data: { plan_id: plan_id, periodo_id, periodo_id },
        success: function(cgts) {
            if (cgts) {
                $.each(cgts, function(key, value) {
                    select.append(new Option(value.gpoGrado + '-' + value.gpoClave + '-' + value.gpoDescripcion, value.id));
                    (value.id == current_value) && select.val(value.id);
                });
                select.trigger('change');
                select.trigger('click');
...
function getGrupos_plan_periodo_preinscripcion_create(plan_id, periodo_id, targetSelect, val = null, dataName = null) {
    var select = $('#' + targetSelect);
    current_data = dataName || 'grupo-id';
    current_value = val || select.data(current_data);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
...
} //getGrupos_plan_periodo_preinscripcion_create.
