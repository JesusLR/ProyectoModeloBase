...
@section('breadcrumbs')
    ...
    <a href="{{url('preescolar_cambiar_contrasenia')}}" class="breadcrumb">Lista Docentes</a>
    <a href="{{url('preescolar_cambiar_contrasenia/create')}}" class="breadcrumb">Crear Contraseña</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
        {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'preescolar.preescolar_cambiar_contrasenia.store', 'method' => 'POST']) !!}
        <div class="card ">
                  ...
                  <div class="col s12 m6 l4">
                    ...
                    <select id="empleado_id" class="browser-default validate select2" required name="empleado_id" style="width: 100%;">
                      ...
                      @foreach ($empleados as $empleado)
                          <option value="{{$empleado->id}}" {{ old('empleado_id') == $empleado->id ? 'selected' : '' }}>{{$empleado->id}}-{{$empleado->perNombre}} {{$empleado->perApellido1}} {{$empleado->perApellido2}}</option>
                      @endforeach                  
                    </select>
                  </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
      ...
      function obtenerCorreoEmpleado(empleado_id) {
          ...
          $.get(base_url+`/preescolar_cambiar_contrasenia/getEmpleadoCorreo/${empleado_id}`, function(res,sta) {
