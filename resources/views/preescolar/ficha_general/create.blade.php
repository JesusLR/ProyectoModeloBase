...
@section('breadcrumbs')
    ...
    <a href="{{url('preescolar_pagos/ficha_general/'.$aluClave.'/'.$perAnioPago.'/'.$programa_id)}}" class="breadcrumb">Referencia de pago</a>
@endsection

@section('content')
    {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'preescolar_pagos.storeFichaGeneral', 'method' => 'POST',  "target" => "_blank"]) !!}
    <div class="card ">
...
<script type="text/javascript">
   ...
   $(document).on('click', '#buscarAlumno', function (e) {
    ...
    $(document).on('click', '#buscarConcepto', function (e) {
      ...
      $.get(base_url + `/preescolar_pagos/ficha_general/obtenerCuotaConcepto/${cuoConcepto}`, function(res, sta) {
        ...
        $("#nomConcepto").val(concepto.progClave + " " + perAnioPago)
        Materialize.updateTextFields();
      });
...
<script type="text/javascript">
    ...
    $("#cuoConcepto").change(function(){
        if($('select[id=cuoConcepto]').val() == "40"){
            ...
            $.get(base_url + `/preescolar_pagos/ficha_general/obtenerAnualidadImporte/${aluClave2}/${cuoAnio2}`, function(result, sta) {
                var importe = result;       
