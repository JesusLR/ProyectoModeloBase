...
<div id="table-datatables">
    ...
    <a href="{{ route('preescolar.preescolar_plan.create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
...
<div class="preloader">
    ...
    <div id="modalCambiarEstadoPreescolar" class="modal">
        <div class="modal-content">
            ...
            <div class="row">
                <div class="col s12">
                    <button type="button" class="guardar-estatus-plan-preescolar btn-large waves-effect  darken-3 btn-flat" style="color: #fff;">
                        <i class="material-icons left">add</i>
                        Guardar
                    </button>
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#tbl-plan').dataTable({
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url+"/preescolar_plan/list",
                beforeSend: function () {
    ...
    $(document).on("click", ".btn-modal-estatus-plan-preescolar", function (e) {
        e.preventDefault()
        $('.modal').modal();
        var planId = $(this).data("plan-id");
        $("#modalCambiarEstadoPreescolar").find(".planId").val(planId);
        getPlan_putPlanEstado(planId, 'planEstado');
    });

    $(document).on("click", ".guardar-estatus-plan-preescolar", function (e) {
        e.preventDefault()

        var planEstado = $("#planEstado").val()
        var planId = $("#modalCambiarEstadoPreescolar").find(".planId").val()
        ...
        if (planEstado && planId) {
            $.ajax({
                ...
                type: "POST",
                dataType: "JSON",
                url: base_url + "/preescolar_plan/cambiarPlanEstado",
            })
    ...
    function getPlan_putPlanEstado(planId, targetSelect) {
        ...
        $.ajax({
            type: 'GET',
            url: base_url + '/preescolar_plan/get_plan/' + planId,
            dataType: 'json',
            data: {'plan_id': planId},
