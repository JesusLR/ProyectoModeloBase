...
<div id="table-datatables">
    ...
    <a href="{{ route('preescolar_empleado.create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
        <i class="material-icons left">add</i>
    </a>
    <a href="{{ route('preescolar_empleado.cambio-estado') }}" class="btn-large waves-effect  darken-3" type="button">Cambio de estado
    ...
    <div class="row">
        <div class="col s12">
            <table id="table-empleados-preescolar" class="responsive-table display" cellspacing="0" width="100%">
                <thead>
                <tr>
                    <th>Clave Docente</th>
                    <th>Nombre(s)</th>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#table-empleados-preescolar').dataTable({
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url+"/preescolar_empleado/list",
                beforeSend: function () {
            ...
            "columns":[
                ...
                {data: "nombre_docente"},
                {data: "apellido1_docente"},
                {data: "apellido2_docente"},
                {data: "empCredencial"},
                {data: "empNomina"},
                {data: "telefono"},
                {data: "empEstado"},
        ...
        $('#table-empleados-preescolar').on('click', '.btn-darBaja-preescolar', function () {

            var empleado_id = $(this).data('id');
            darBaja(empleado_id);
        });

        $('#table-empleados-preescolar').on('click', '.btn-borrar', function () {
            var empleado_id = $(this).data('id');
    ...
    function darBaja(empleado_id) {
        ...
        swal({
            ...
            function () {
                ...
                $.ajax({
                    type: 'POST',
                    url: base_url + '/preescolar_empleado/darBaja/' + empleado_id,
                    dataType: 'json',
    ...
    function borrar_empleado(empleado_id) {
        $.ajax({
            type: 'GET',
            url: base_url + '/preescolar_empleado/verificar_delete/'+empleado_id,
            dataType: 'json',
