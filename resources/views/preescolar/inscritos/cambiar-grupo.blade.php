...
<div class="row">
  <div class="col s12 ">
    {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'PreescolarInscritos.postCambiarGrupo', 'method' => 'POST']) !!}
              ...
              <div class="col s12 m6 l4">
                ...
                <select name="gpoId" id="gpoId" class="browser-default validate select2" style="width: 100%;">
                    @foreach($grupos as $key => $value)
                      <option value="{{$value->id}}" {{$inscrito->preescolar_grupo->id == $value->id ? "selected": ""}}>
                          {{$value->preescolar_materia->matNombre}} - {{$value->gpoGrado}}{{$value->gpoClave}}{{$value->gpoTurno}}
                      </option>
                    @endforeach
                </select>
              </div>
