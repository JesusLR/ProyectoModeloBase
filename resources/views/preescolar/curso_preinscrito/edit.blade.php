...
@section('breadcrumbs')
    ...
    <a href="{{url('preescolar_curso')}}" class="breadcrumb">Lista de Preinscripción</a>
    <a href="{{url('preescolar_curso/'.$curso->id.'/edit')}}" class="breadcrumb">Editar preinscripción</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
        {{ Form::open(['enctype' => 'multipart/form-data', 'method'=>'PUT','route' => ['curso_preescolar.update', $curso->id]]) }}
        <div class="card ">
          <div class="card-content ">
            <span class="card-title">EDITAR PREINSCRIPCIÓN #{{$curso->id}}</span>
                ...
                <div class="row">
                    ...
                    <div class="col s12 m6 l4">
                        ...
                        <select id="cgt_id" class="browser-default validate select2" required name="cgt_id" style="width: 100%;">
                            <option value="{{$cgt_actual->id}}">{{$cgt_actual->cgtGradoSemestre.'-'.$cgt_actual->cgtGrupo.'-'.$cgt_actual->cgtTurno}}</option>
                            {{--  @foreach ($cgts as $cgt)
                ...
                <div class="row">
                    @if($permiso == 'A' || $permiso == 'B')
                        <div class="col s12 m6 l4">
                            ...
                            <select name="curEstado" id="curEstado" required class="browser-default validate select2" style="width: 100%;">
                                ...
                                @endforeach  --}}
                                <option value="{{$curso->curEstado}}">
                                    @if ($curso->curEstado == "P") PREINSCRITO @endif
                                    @if ($curso->curEstado == "R") REGULAR @endif
                                    @if ($curso->curEstado == "C") CONDICIONADO @endif
                                    @if ($curso->curEstado == "A") CONDICIONADO 2 @endif
                                    @if ($curso->curEstado == "B") BAJA @endif
                                </option>
                            </select>
                        </div>
                    @endif
                    <div class="col s12 m6 l4">
                        ...
                        <select name="curTipoIngreso" id="curTipoIngreso" required class="browser-default validate select2" style="width: 100%;">
                            <option value="{{$curso->curTipoIngreso}}">
                                @if ($curso->curTipoIngreso == "PI") PRIMER INGRESO @endif
                                @if ($curso->curTipoIngreso == "NI") NUEVO INGRESO @endif
                                @if ($curso->curTipoIngreso == "RO") REPETIDOR @endif
                                @if ($curso->curTipoIngreso == "RI") REINSCRIPCIÓN @endif
                                @if ($curso->curTipoIngreso == "RE") REINGRESO @endif
                                @if ($curso->curTipoIngreso == "EQ") REVALIDACIÓN @endif
                                @if ($curso->curTipoIngreso == "OY") OYENTE @endif
                                @if ($curso->curTipoIngreso == "XX") OTRO @endif
                            </option>
