...
@section('breadcrumbs')
    ...
    <a href="{{route('curso_preescolar.index')}}" class="breadcrumb">Lista de preinscritos</a>
@endsection
...
<div id="table-datatables">
    <h4 class="header">PREINSCRITOS MATERNAL | PREESCOLAR</h4>
    @php use App\Models\User; @endphp
    @if (User::permiso("curso") != "D" && User::permiso("curso") != "P")
        <a href="{{ route('curso_preescolar.create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
    ...
    <div class="row">
        <div class="col s12">
            <table id="tbl-curso-preescolar" class="responsive-table display" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        <th>Año</th>
                        <th>Clave Alumno</th>
...
@include('preescolar.curso_preinscrito.modales.modaHistorialPagos-preescolar')
@include('preescolar.curso_preinscrito.modales.modaBajaCurso-preescolar')
@include('modales.modalBajaARegular')
@include('preescolar.curso_preinscrito.modales.modalAlumnoDetalle-preescolar')
...
<script type="text/javascript">
    $(document).on("click", ".btn-modal-ficha-pago-sinvalidar", function(e) {
        e.preventDefault()
...
<script type="text/javascript">
    $(document).on("click", ".btn-modal-ficha-pago-hsbc-sinvalidar", function(e) {
        e.preventDefault()
...
<script type="text/javascript">
    function modalHistorialPagos(curso_id) {
        ...
        $('#tbl-historial-pagos-preescolar').dataTable({
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/api/preescolar_curso/listHistorialPagos/" + curso_id,
                beforeSend: function () {
    ...
    function modalHistorialPagosAluClave(aluClave) {
        ...
        $('#tbl-historial-pagos-alu-preescolar').dataTable({
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/preescolar_alumnos/listHistorialPagosAluclave/" + aluClave,
                beforeSend: function () {
                    $('.preloader-modal').fadeIn(200, function() {
                        $(this).append('<div id="preloader-modal"></div>');
                    });
...
$(document).on('click', '.confirmar-baja-alumno', function (e) {
    ...
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: `${base_url}/api/curso/${curso_id}/verificar_materias_cargadas`,
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-baja-curso", function (e) {
            ...
            $.get(base_url+`/api/preescolar_curso/conceptosBaja`, function(res,sta) {
                res.forEach(element => {
...
<script type="text/javascript">
    $(document).ready(function() {
        $.fn.dataTable.ext.errMode = 'throw';
        var tableCurso = $('#tbl-curso-preescolar').dataTable({
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            "pageLength": 5,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url +"/preescolar_curso/list",
                beforeSend: function () {
                    $('.preloader').fadeIn(200,function(){$(this).append('<div id="preloader"></div>');});
                },
                complete: function () {
