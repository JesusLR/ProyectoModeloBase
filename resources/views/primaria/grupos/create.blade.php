...
@section('breadcrumbs')
    ...
    <a href="{{ route('primaria_grupo.create') }}" class="breadcrumb">Agregar Grupo</a>
@endsection
            ...
            <div id="general">
                ...
                <div class="row">                    
                    <div class="col s12 l6">
                        {!! Form::label('primaria_materia_asignatura_id', 'Materia complementaria *', array('class' => '')); !!}
                        <select id="primaria_materia_asignatura_id" class="browser-default validate select2" name="primaria_materia_asignatura_id" style="width: 100%;" disabled>
                            {{--  <option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>  --}}
                        </select>
                    </div>
                    <input type="hidden" name="gpoMatComplementaria" id="gpoMatComplementaria" value="">
                </div>
                {{--  <div class="row">
                    <div class="col s12 m6 l4">
                        <div class="input-field">
                        {!! Form::text('nombreAlternativo', NULL, array('id' => 'nombreAlternativo','onKeyPress="if(this.value.length==75) return false;"')) !!}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function postGuardarGrupoPrimaria () {
            $.ajax({
                data: {
                    ...
                    "primaria_materia_asignatura_id": $('#primaria_materia_asignatura_id').val()

                },
                type: "POST",
                dataType: "JSON",
                url: base_url + "/primaria_grupo",
            })
                ...
                if (!data.res) {
                    ...
                    if (data.existeGrupo) {
                        ...
                        html += "<p style='text-align:left;'><b>Materia:</b>"
                            + " " + data.msg.primaria_materia.matNombre
                        + "</p>"
                        html += "<p style='text-align:left;'><b>Maestro:</b>"
                            + " " + data.msg.primaria_empleado.empNombre
                            + " " + data.msg.primaria_empleado.empApellido1
                            + " " + data.msg.primaria_empleado.empApellido2
                        + "</p>"
                        html += "<p>" + "</p>"
...
@include('primaria.scripts.grupos')
...
<script>
    $("input[name=gpoACD]").change(function(){
        if ($(this).is(':checked') ) {
            ...
            $("#primaria_materia_asignatura_id").prop('required', true);
            $("#primaria_materia_asignatura_id").prop('disabled', false);
            ...
            $("#materia_id").change( event => {
                ...
                $("#primaria_materia_asignatura_id").empty();
                $("#primaria_materia_asignatura_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/primaria_grupo/materiaComplementaria/${event.target.value}/${plan_id}/${periodo_id}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#primaria_materia_asignatura_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            });  
            ...
            $("#periodo_id").change( event => {
                ...
                $("#primaria_materia_asignatura_id").empty();
                $("#primaria_materia_asignatura_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/primaria_grupo/materiaComplementaria/${materia_id}/${plan_id}/${event.target.value}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#primaria_materia_asignatura_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            $("#plan_id").change( event => {
                ...
                $("#primaria_materia_asignatura_id").empty();
                $("#primaria_materia_asignatura_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/primaria_grupo/materiaComplementaria/${materia_id}/${event.target.value}/${periodo_id}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#primaria_materia_asignatura_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            var plan_id = $("#plan_id").val();
                ...
                $("#primaria_materia_asignatura_id").empty();
                $("#primaria_materia_asignatura_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/primaria_grupo/materiaComplementaria/${materia_id}/${plan_id}/${periodo_id}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#primaria_materia_asignatura_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            $("#primaria_materia_asignatura_id").change(function(){
                $('#gpoMatComplementaria').val($('select[name="primaria_materia_asignatura_id"] option:selected').text());
            ...
            $("#primaria_materia_asignatura_id").prop('required', false);
            $("#primaria_materia_asignatura_id").prop('disabled', true);
