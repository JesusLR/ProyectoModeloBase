...
@section('breadcrumbs')
    ...
    <a href="{{ url('primaria_grupo/'.$grupo->id.'/evidencia') }}" class="breadcrumb">Agregar evidencia</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'primaria_grupo.guardar_actualizar_evidencia', 'method' => 'POST']) !!}
        <div class="card ">
          <div class="card-content ">
            ...
            {{-- GENERAL BAR--}}
            <div id="general">
                {{-- <input type="hidden" name="primaria_grupo_id" value="{{$grupo->id}}"> --}}
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        ...
                        <p><b>Materia: </b>{{$grupo->primaria_materia->matNombre}}</p>
                        @if ($grupo->primaria_materia_asignatura != null)
                        <p><b>Asignatura: </b>{{$grupo->primaria_materia_asignatura->matClaveAsignatura.'-'.$grupo->primaria_materia_asignatura->matNombreAsignatura}}</p>
                        @endif
                        <p><b>Grado-Grupo: </b>{{$grupo->gpoGrado}}-{{$grupo->gpoClave}}</p>
                        <p><b>Docente: </b>{{$grupo->primaria_empleado->empNombre}} {{$grupo->primaria_empleado->empApellido1}} {{$grupo->primaria_empleado->empApellido2}}</p>
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        {!! Form::label('primaria_mes_evaluacion_id', 'Mes *', array('class' => '')); !!}
                        <select id="primaria_mes_evaluacion_id" class="browser-default validate select2" required name="primaria_mes_evaluacion_id" style="width: 100%;">
                            <option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>
                            @if ($Evidencias != "")
                                @foreach ($meses as $mes)
                                    <option value="{{$mes->id}}" {{ $Evidencias->primaria_mes_evaluacion_id == $mes->id ? 'selected="selected"' : '' }}>{{$mes->mes}}</option>
                                @endforeach
                            @else
                                @foreach ($meses as $mes)
                    ...
                    <div class="col s12 m6 l4" style="margin-top: -10px">
                        <div class="input-field">
                            ...
                            <input type="number" onclick="cuentaTotalEvidencia()" onKeyDown="cuentaTotalEvidencia()" onKeyUp="cuentaTotalEvidencia()" name="numero_evidencias" id="numero_evidencias" min="2" max="10" required>
                        </div>
                    </div>
...
@include('primaria.grupos.funcionesJS')
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function obtenerDatos(grupo_id,mes_id) {
            ...
            $.get(base_url+`/api/getEvidencias/${grupo_id}/${mes_id}`, function(res,sta) {
                ...
                if(res != ""){
                    res.forEach(element => {
                        ...
                        //validamos si es editable o no
                        if(element.evidencia1_editable == "1"){
                            $('#concepto_evidencia1').prop('readonly', false);
                            $('#porcentaje_evidencia1').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia1').prop('readonly', true);
                            $('#porcentaje_evidencia1').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia2_editable == "1"){
                            $('#concepto_evidencia2').prop('readonly', false);
                            $('#porcentaje_evidencia2').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia2').prop('readonly', true);
                            $('#porcentaje_evidencia2').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia3_editable == "1"){
                            $('#concepto_evidencia3').prop('readonly', false);
                            $('#porcentaje_evidencia3').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia3').prop('readonly', true);
                            $('#porcentaje_evidencia3').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia4_editable == "1"){
                            $('#concepto_evidencia4').prop('readonly', false);
                            $('#porcentaje_evidencia4').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia4').prop('readonly', true);
                            $('#porcentaje_evidencia4').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia5_editable == "1"){
                            $('#concepto_evidencia5').prop('readonly', false);
                            $('#porcentaje_evidencia5').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia5').prop('readonly', true);
                            $('#porcentaje_evidencia5').prop('readonly', true);
                        }


                        //validamos si es editable o no
                        if(element.evidencia6_editable == "1"){
                            $('#concepto_evidencia6').prop('readonly', false);
                            $('#porcentaje_evidencia6').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia6').prop('readonly', true);
                            $('#porcentaje_evidencia6').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia7_editable == "1"){
                            $('#concepto_evidencia7').prop('readonly', false);
                            $('#porcentaje_evidencia7').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia7').prop('readonly', true);
                            $('#porcentaje_evidencia7').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia8_editable == "1"){
                            $('#concepto_evidencia8').prop('readonly', false);
                            $('#porcentaje_evidencia8').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia8').prop('readonly', true);
                            $('#porcentaje_evidencia8').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia9_editable == "1"){
                            $('#concepto_evidencia9').prop('readonly', false);
                            $('#porcentaje_evidencia9').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia9').prop('readonly', true);
                            $('#porcentaje_evidencia9').prop('readonly', true);
                        }

                        //validamos si es editable o no
                        if(element.evidencia10_editable == "1"){
                            $('#concepto_evidencia10').prop('readonly', false);
                            $('#porcentaje_evidencia10').prop('readonly', false);
                        }else{
                            $('#concepto_evidencia10').prop('readonly', true);
                            $('#porcentaje_evidencia10').prop('readonly', true);
                        }
        ...
        obtenerDatos($("#primaria_grupo_id").val(),$("#primaria_mes_evaluacion_id").val())
        $("#primaria_mes_evaluacion_id").change( event => {
            obtenerDatos($("#primaria_grupo_id").val(),event.target.value)
        });
     });
  </script>
