...
@section('breadcrumbs')
    ...
    <a href="{{url('primaria_fecha_publicacion_calificacion_alumno/'.$primaria_calendario_calificaciones_alumnos->id.'/edit')}}" class="breadcrumb">Editar fecha de captura calificacion alumno</a>
@endsection

@section('content')
<div class="row">
    <div class="col s12 ">
        {{ Form::open(['enctype' => 'multipart/form-data', 'method'=>'PUT','route' => ['primaria.primaria_fecha_publicacion_calificacion_alumno.update', $primaria_calendario_calificaciones_alumnos->id]]) }}
        <div class="card ">
                ...
                <div id="general">
                        ...
                        <div class="col s12 m6 l4">
                            ...
                            <select id="primaria_mes_evaluaciones_id" data-plan-id="{{old('primaria_mes_evaluaciones_id')}}" class="browser-default validate select2" required name="primaria_mes_evaluaciones_id"
                                ...
                                @foreach ($primaria_mes_evaluaciones as $mes_evaluacion)
                                    <option value="{{$mes_evaluacion->id}}" {{ $mes_evaluacion->id == $primaria_calendario_calificaciones_alumnos->primaria_mes_evaluaciones_id ? 'selected' : '' }}>{{$mes_evaluacion->mes}}</option>
                                @endforeach
                            </select>
                        </div>                
                    </div>
                    ...
                    <div class="row">
                        <div class="col s12 m6 l4">
                            ...
                            {!! Form::date('calPublicacion', \Carbon\Carbon::parse($primaria_calendario_calificaciones_alumnos->calPublicacion)->format('Y-m-d'), array('id' => 'calPublicacion', 'class' => 'validate')) !!}
                        </div>
