...
@section('breadcrumbs')
    ...
    <a href="{{route('secundaria.secundaria_curso.index')}}" class="breadcrumb">Lista de preinscritos</a>
@endsection
...
<div id="table-datatables">
    <h4 class="header">PREINSCRITOS SECUNDARIA</h4>
    @php use App\Models\User; @endphp
    @if (User::permiso("curso") != "D" && User::permiso("curso") != "P")
        <a href="{{ route('secundaria.secundaria_curso.create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
    ...
    <div class="row">
        <div class="col s12">
            <table id="tbl-curso-secundaria" class="responsive-table display" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        <th>Año</th>
                        <th>Clave Alumno</th>
...
@include('secundaria.cursos.modales.modaHistorialPagos-secundaria')
@include('secundaria.cursos.modales.modaBajaCurso-secundaria')
@include('secundaria.cursos.modales.modalBajaARegular')
@include('secundaria.cursos.modales.modalAlumnoDetalle-secundaria')
...
<script type="text/javascript">
    $(document).on("click", ".btn-modal-ficha-pago-sin-validar", function(e) {
        e.preventDefault()
...
<script type="text/javascript">
    $(document).on("click", ".btn-modal-ficha-pago-hsbc-sin-validar", function(e) {
        e.preventDefault()
...
<script type="text/javascript">
    function modalHistorialPagos(curso_id) {
        ...
        $('.modal').modal();
        //MOSTRAR GRUPOS
        if ($.fn.DataTable.isDataTable("#tbl-historial-pagos-secundaria")) {
            $('#tbl-historial-pagos-secundaria').DataTable().clear().destroy();
        }

        $.fn.dataTable.ext.errMode = 'throw';
        $('#tbl-historial-pagos-secundaria').dataTable({
            "destroy": true,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/secundaria_curso/listHistorialPagos/" + curso_id,
                beforeSend: function () {
    ...
    function modalHistorialPagosAluClave(aluClave) {
        ...
        $('.modal').modal();
        //MOSTRAR GRUPOS
        if ($.fn.DataTable.isDataTable("#tbl-historial-pagos-alu-secundaria")) {
            $('#tbl-historial-pagos-alu-secundaria').DataTable().clear().destroy();
        }

        $.fn.dataTable.ext.errMode = 'throw';
        $('#tbl-historial-pagos-alu-secundaria').dataTable({
            "destroy": true,
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url + "/secundaria_alumno/listHistorialPagosAluclave/" + aluClave,
                beforeSend: function () {
                    $('.preloader-modal').fadeIn(200, function() {
                        $(this).append('<div id="preloader-modal"></div>');
                    });
    ...
    $(document).on("click", ".btn-modal-historial-pagos-secundaria", function (e) {
        e.preventDefault()
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-preinscrito-detalle", function (e) {
            ...
            $.get(base_url+`/secundaria_curso/api/curso/${curso_id}`, function(res,sta) {
...
$(document).on('click', '.confirmar-baja-alumno', function (e) {
    ...
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: `${base_url}/secundaria_curso/api/curso/${curso_id}/verificar_materias_cargadas`,
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", ".btn-modal-baja-curso-secundaria", function (e) {
            e.preventDefault()
            var curso_id = $(this).data("curso-id");
            $(".modalCursoId").val(curso_id)

            $.get(base_url+`/secundaria_curso/api/curso/infoBaja/` + curso_id, function(res, sta) {
                console.log(res);
            ...
            $.get(base_url+`/secundaria_curso/conceptosBaja`, function(res,sta) {
                res.forEach(element => {
            ...
            $('#tbl-posibles-hermanos').dataTable({
                ...
                "ajax": {
                    "type" : "GET",
                    'url': "secundaria_curso/listPosiblesHermanos/" + curso_id,
                    beforeSend: function () {
...
<script type="text/javascript">
    $(document).ready(function() {
        $.fn.dataTable.ext.errMode = 'throw';
        var tableCurso = $('#tbl-curso-secundaria').dataTable({
            "language":{"url":"api/lang/javascript/datatables"},
            "serverSide": true,
            "dom": '"top"i',
            "pageLength": 5,
            ...
            "ajax": {
                "type" : "GET",
                'url': base_url +"/secundaria_curso/list",
                beforeSend: function () {
                    $('.preloader').fadeIn(200,function(){$(this).append('<div id="preloader"></div>');});
                },
                complete: function () {
            ...
            "columns":[
                ...
                {data: "genero"},
                {data: "estado_curso"},
                {data: "curFechaBaja", render: function(data, type,row) {
                    if (row.curEstado == 'B') {
                        return data;
                    } else return null;
                }},
                ...
                {data: "plan_pago"},
                {data: "beca"},
                {data: "clave_ubicacion"},
                {data: "clave_departamento"},
                {data: "clave_escuela"},
                {data: "clave_programa"},
                {data: "clave_plan"},
                {data: "action"}
