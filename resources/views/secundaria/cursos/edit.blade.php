...
@section('breadcrumbs')
    ...
    <a href="{{url('secundaria_curso')}}" class="breadcrumb">Lista de Preinscripción</a>
    <a href="{{url('secundaria_curso/'.$curso->id.'/edit')}}" class="breadcrumb">Editar preinscripción</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
        {{ Form::open(['enctype' => 'multipart/form-data', 'method'=>'PUT','route' => ['secundaria.secundaria_curso.update', $curso->id]]) }}
        <div class="card ">
                ...
                <div class="row">
                    ...
                    <div class="col s12 m6 l8">
                        <div class="file-field input-field">
                            ...
                            <input value="" type="file" name="curSecundariaFoto">
                            </div>
                        ...
                        @if ($curso->curSecundariaFoto)
                            <img style="width:200px;" src="{{url('/secundaria_curso_images/' . $curso->curSecundariaFoto) }}" alt="">
                        @endif
                    </div>
                    <div class="col s12 m6 l4" style="visibility: hidden;">
                        <div class="input-field">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#departamento_id").change( event => {
            ...
            $.get(base_url+`/secundaria_periodo/api/periodos/${event.target.value}`,function(res2,sta){
                var perSeleccionado;
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#periodo_id").change( event => {
            ...
            Materialize.updateTextFields();
            $.get(base_url+`/secundaria_periodo/api/periodo/${event.target.value}`,function(res,sta){
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#plan_id").change( event => {
            ...
            $("#materia_id").empty();
            $("#materia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $.get(base_url+`/secundaria_cgt/api/cgts/${event.target.value}/${periodo_id}`,function(res,sta){
                res.forEach(element => {
        ...
        $("#periodo_id").change( event => {
            ...
            $("#cgt_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $("#materia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $.get(base_url+`/secundaria_cgt/api/cgts/${plan_id}/${event.target.value}`,function(res,sta){
                res.forEach(element => {
                    $("#cgt_id").append(`<option value=${element.id}>${element.cgtGradoSemestre}-${element.cgtGrupo}-${element.cgtTurno}</option>`);
                });
            });
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#gpoSemestre").change( event => {
            ...
            $("#materia_id").empty();
            $("#materia_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
            $.get(base_url+`/secundaria_materia/materias/${event.target.value}/${plan_id}`,function(res,sta){
                res.forEach(element => {
