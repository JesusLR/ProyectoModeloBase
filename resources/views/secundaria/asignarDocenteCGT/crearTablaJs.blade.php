<script type="text/javascript">
    $(document).ready(function() {
        ...
        function obtenerSecundariaGrupos(periodo_id, plan_id,gpoGrado, gpoGrupo) {
            $.get(base_url+`/secundaria_asignar_docente/obtenerGrupos/${periodo_id}/${plan_id}/${gpoGrado}/${gpoGrupo}`, function(res,sta) {
        ...
        obtenerSecundariaGrupos($("#periodo_id").val(),$("#plan_id").val(),$("#gpoGrado").val(),$("#gpoGrupo").val())        
        $("#periodo_id").change( eventPerido => {
            $("#plan_id").change( eventPlan => {
                $("#gpoGrado").blur( eventGru => {
                    $("#gpoGrupo").blur( eventGrad => {
                        obtenerSecundariaGrupos(eventPerido.target.value, eventPlan.target.value, eventGru.target.value, eventGrad.target.value)
                    });
                });
            });
        });
