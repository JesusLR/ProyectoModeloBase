...
@section('breadcrumbs')
    ...
    <a href="{{ route('secundaria.secundaria_grupo.create') }}" class="breadcrumb">Agregar Grupo</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
      {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'secundaria.secundaria_grupo.store', 'method' => 'POST']) !!}
        <div class="card ">
          <div class="card-content ">
            ...
            <div id="general">
                ...
                <div class="row">                    
                    <div class="col s12 l6">
                        {!! Form::label('secundaria_materia_acd_id', 'Materia complementaria *', array('class' => '')); !!}
                        <select id="secundaria_materia_acd_id" class="browser-default validate select2" name="secundaria_materia_acd_id" style="width: 100%;" disabled>
                            {{--  <option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>  --}}
                        </select>
                    </div>

                    <input type="hidden" value="" name="gpoMatComplementaria" id="gpoMatComplementaria">
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function postGuardarGrupoSecundaria () {
            $.ajax({
                data: {
                    ...
                    "secundaria_materia_acd_id": $('#secundaria_materia_acd_id').val()
                ...
                if (!data.res) {
                    ...
                    if (data.existeGrupo) {
                        ...
                        html += "<p style='text-align:left;'><b>Materia:</b>"
                            + " " + data.msg.secundaria_materia.matNombre
                        + "</p>"
                        html += "<p style='text-align:left;'><b>Maestro:</b>"
                            + " " + data.msg.secundaria_empleado.empNombre
                            + " " + data.msg.secundaria_empleado.empApellido1
                            + " " + data.msg.secundaria_empleado.empApellido2
                        + "</p>"
                        html += "<p>" + "</p>"
...
<script>
    $("input[name=gpoACD]").change(function(){
        if ($(this).is(':checked') ) {
            ...
            $("#secundaria_materia_acd_id").prop('required', true);
            $("#secundaria_materia_acd_id").prop('disabled', false);
            ...
            $("#materia_id").change( event => {
                ...
                $("#secundaria_materia_acd_id").empty();
                $("#secundaria_materia_acd_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/secundaria_grupo/materiaComplementaria/${event.target.value}/${plan_id}/${periodo_id}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#secundaria_materia_acd_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            $("#periodo_id").change( event => {
                ...
                $("#secundaria_materia_acd_id").empty();
                $("#secundaria_materia_acd_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/secundaria_grupo/materiaComplementaria/${materia_id}/${plan_id}/${event.target.value}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#secundaria_materia_acd_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            $("#plan_id").change( event => {
                ...
                $("#secundaria_materia_acd_id").empty();
                $("#secundaria_materia_acd_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/secundaria_grupo/materiaComplementaria/${materia_id}/${event.target.value}/${periodo_id}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#secundaria_materia_acd_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            var plan_id = $("#plan_id").val();
                ...
                $("#secundaria_materia_acd_id").empty();
                $("#secundaria_materia_acd_id").append(`<option value="" selected disabled>SELECCIONE UNA OPCIÓN</option>`);
                $.get(base_url+`/secundaria_grupo/materiaComplementaria/${materia_id}/${plan_id}/${periodo_id}/${gpoSemestre}`,function(resACD,sta){
                    resACD.forEach(materiaACD => {
                        $("#secundaria_materia_acd_id").append(`<option value="${materiaACD.id}">${materiaACD.gpoMatComplementaria}</option>`);
                    });
                });
            ...
            $("#secundaria_materia_acd_id").change(function(){
                $("#gpoMatComplementaria").val($('select[name="secundaria_materia_acd_id"] option:selected').text());
            });
            ...
            $("#secundaria_materia_acd_id").prop('required', false);
            $("#secundaria_materia_acd_id").prop('disabled', true);
