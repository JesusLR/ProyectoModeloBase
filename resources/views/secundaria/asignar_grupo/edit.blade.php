...
@section('breadcrumbs')
    ...
    <a href="{{url('secundaria_asignar_grupo/'.$inscrito->id.'/edit')}}" class="breadcrumb">Editar inscrito materia</a>
@endsection
...
<div class="row">
    <div class="col s12 ">
        {{ Form::open(array('method'=>'PUT','route' => ['secundaria.secundaria_asignar_grupo.update', $inscrito->id])) }}
        <div class="card ">
                    ...
                    <div class="col s12 ">
                        ...
                        <select id="grupo_id" class="browser-default validate select2" required name="grupo_id" style="width: 100%;">
                            ...
                            @foreach($grupos as $grupo)
                                <option value="{{$grupo->id}}" @if($inscrito->grupo_id == $grupo->id) {{ 'selected' }} @endif>{{"Grupo: ".$grupo->gpoGrado.'-'.$grupo->gpoClave.'-'.$grupo->gpoTurno.' '.$grupo->gpoMatComplementaria. ", Materia: ".$grupo->secundaria_materia->matClave.'-'.$grupo->secundaria_materia->matNombre.' '."Maestro: ".$grupo->secundaria_empleado->empNombre.' '.$grupo->secundaria_empleado->empApellido1.' '.$grupo->secundaria_empleado->empApellido2}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#curso_id").change( event => {
            ...
            $.get(base_url+`/secundaria_asignar_grupo/grupos/${event.target.value}`, function(res, sta) {
                ...
                res.forEach(element => {
                    ...
                    if(element.gpoMatComplementaria == null){
                        ...
                        var gpoMatComplementaria = element.gpoMatComplementaria;
                    }

                    if(element.empNombre == null){
                        var nombreDocente = element.empApellido1;
                    }else{
                        var nombreDocente = `${element.empNombre} ${element.empApellido1} ${element.empApellido2}`;
                    }


                    if (!element.optNombre) {
                        $("#grupo_id").append(`<option value=${element.id}>
                            Grupo: ${element.gpoGrado}-${element.gpoClave}-${element.gpoTurno} ${gpoMatComplementaria},
                            Materia: ${element.matClave}-${element.matNombre}
                            Maestro: ${nombreDocente}
                        </option>`);
                    }
