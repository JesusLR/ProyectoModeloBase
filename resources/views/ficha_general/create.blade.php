...
@section('breadcrumbs')
    ...
    <a href="{{url('aex_pagos/ficha_general/'.$aluClave.'/'.$perAnioPago.'/'.$programa_id.'/'.$ubiNombre.'/'.$perNumero)}}" class="breadcrumb">Referencia de pago</a>
@endsection

@section('content')
    {!! Form::open(['onKeypress' => 'return disableEnterKey(event)','route' => 'aex_pagos.storeFichaGeneral', 'method' => 'POST',  "target" => "_blank"]) !!}
    <div class="card ">
        ...
        <div id="general">
            <div class="row">
                ...
                <div class="col s12 l3">
                    ...
                    {!! Form::text('aluClave', $aluClave, array('id' => 'aluClave', 'class' => 'validate')) !!}
                    {!! Form::label('aluClave', 'Clave de alumno *', ['class' => '']); !!}
                    </div>
                </div>
                <div class="col s12 l3">
                    <div class="input-field">
                    {!! Form::number('cuoAnio', $perAnioPago, array('id' => 'cuoAnio', 'class' => 'validate','maxLength' => '4','required')) !!}
            ...
            <div class="row">
                <div class="col s12 m6 l4">
                    <div class="input-field">
                    {!! Form::text('ubiNombre', $ubiNombre, array('id' => 'ubiNombre', 'class' => 'validate','readonly')) !!}
                    {!! Form::label('ubiNombre', 'Ubicación (solo lectura)', ['class' => '']); !!}
                    </div>
                </div>
                <div class="col s12 m6 l4">
                    <div class="input-field">
                    {!! Form::text('perNumero', $perNumero, array('id' => 'perNumero', 'class' => 'validate','readonly')) !!}
            ...
            <div>
                <div class="col s12">
                    <div class="input-field">
                    {!! Form::text('aluNombre', $alumno->perNombre.' '.$alumno->perApellido1.' '.$alumno->perApellido2, array('id' => 'aluNombre', 'class' => 'validate','readonly')) !!}
            ...
            <div class="row">
                <div class="col s12 m6 l3">
                    ...
                    <select id="cuoConcepto" class="browser-default validate select2" required name="cuoConcepto" style="width: 100%;">
                        ...
                        @if ($conceptosPago1 != "")
                            <option value="{{$conceptosPago1->conpClave}}">{{$conceptosPago1->conpClave}} {{$conceptosPago1->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago2 != "")
                            <option value="{{$conceptosPago2->conpClave}}">{{$conceptosPago2->conpClave}} {{$conceptosPago2->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago3 != "")
                            <option value="{{$conceptosPago3->conpClave}}">{{$conceptosPago3->conpClave}} {{$conceptosPago3->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago4 != "")
                            <option value="{{$conceptosPago4->conpClave}}">{{$conceptosPago4->conpClave}} {{$conceptosPago4->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago5 != "")
                            <option value="{{$conceptosPago5->conpClave}}">{{$conceptosPago5->conpClave}} {{$conceptosPago5->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago6 != "")
                            <option value="{{$conceptosPago6->conpClave}}">{{$conceptosPago6->conpClave}} {{$conceptosPago6->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago7 != "")
                            <option value="{{$conceptosPago7->conpClave}}">{{$conceptosPago7->conpClave}} {{$conceptosPago7->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago8 != "")
                            <option value="{{$conceptosPago8->conpClave}}">{{$conceptosPago8->conpClave}} {{$conceptosPago8->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago9 != "")
                            <option value="{{$conceptosPago9->conpClave}}">{{$conceptosPago9->conpClave}} {{$conceptosPago9->conpNombre}}</option>
                        @endif
                        @if ($conceptosPago10 != "")
                            <option value="{{$conceptosPago10->conpClave}}">{{$conceptosPago10->conpClave}} {{$conceptosPago10->conpNombre}}</option>
                        @endif                          
                    </select>
            ...
            <div class="row">
                <div class="col s12 m6 l3">
                    <div class="input-field">
                    {!! Form::text('importeNormal', $actividad->actImporte, array('id' => 'importeNormal', 'class' => 'validate','required')) !!}
...
<script type="text/javascript">
    ...
    $(document).on('click', '#buscarConcepto', function (e) {
      ...
      $.get(base_url + `/aex_pagos/ficha_general/obtenerCuotaConcepto/${cuoConcepto}`, function(res, sta) {

        var concepto = JSON.parse(res)

        var perAnioPago = $("#cuoAnio").val();
...
<script type="text/javascript">
    ...
    $("#cuoConcepto").change(function(){
        if($('select[id=cuoConcepto]').val() == "40"){
            ...
            $.get(base_url + `/aex_pagos/ficha_general/obtenerAnualidadImporte/${aluClave2}/${cuoAnio2}`, function(result, sta) {
                var importe = result;       
            ...
            $("#importeNormal").val("{{$actividad->actImporte}}")
