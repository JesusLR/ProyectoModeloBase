...
@section('content')
    <div id="table-datatables">
        ...
        <a href="{{ url('/alumno/create') }}" class="btn-large waves-effect  darken-3" type="button">Agregar
        ...
        <div class="row">
            <div class="col s12">
                <table id="tbl-alumno" class="responsive-table display" cellspacing="0" width="100%">
                    <thead>
                    <tr>
                        <th>Clave Alumno</th>
    ...
    <div id="modalEstatusAlumno" class="modal">
    ...
    {{-- MODAL BAJAS ALUMNO --}}
    <div id="modalHistorialBajasAlu" class="modal">
        <div class="modal-content">
            <div class="row">
                <div class="col s12 m6 l6">
                    <h4>Historial de pagos</h4>
            ...
            <table id="tbl-historial-bajas" class="responsive-table display" cellspacing="0" width="100%">
                <thead>
                <tr>
                    <th>Curso</th>
                    <th>Concepto de pago</th>
...
<script type="text/javascript">
        function modalHistorialBajas(aluClave) {
            //MOSTRAR MODAL
            $('.modal').modal();
            if ($.fn.DataTable.isDataTable("#tbl-historial-bajas")) {
                $('#tbl-historial-bajas').DataTable().clear().destroy();
            }

            $.fn.dataTable.ext.errMode = 'throw';
            $('#tbl-historial-bajas').dataTable({
                "destroy": true,
                "language":{"url":"api/lang/javascript/datatables"},
                "serverSide": true,
                "dom": '"top"i',
                ...
                "ajax": {
                    "type" : "GET",
                    'url': base_url + "/api/alumno/listHistorialBajasAluclave/" + aluClave,
                    beforeSend: function () {
                        $('.preloader-modal').fadeIn(200, function() {
                            $(this).append('<div id="preloader-modal"></div>');
                        });
                ...
                //Apply the search
                initComplete: function () {
                    console.log('DataTables_' + this.api().context[0].sInstance);
                    var searchFill = JSON.parse(localStorage.getItem( 'DataTables_' + this.api().context[0].sInstance ))
        ...
        $(document).on("click", ".btn-modal-historial-bajas", function (e) {
            e.preventDefault()
            var aluClave = $(this).data("aluclave");
            var nombres = $(this).data("nombres");
            var alumno_id = $(this).data('alumno-id');
    ...
    <script>
        $(document).on("click", ".btn-modal-estatus-alumno", function (e) {
            ...
            $("#modalEstatusAlumno").find(".alumnoId").val(alumnoId)
        ...
        $(document).on("click", ".guardar-estatus-alumno", function (e) {
            ...
            var alumnoId = $("#modalEstatusAlumno").find(".alumnoId").val()
            var resFechaBaja = $('#resFechaBaja').val();
            var resObservaciones = $('#resObservaciones').val();
            var conceptosBaja = $('#conceptosBaja').val();
            ...
            if (aluEstado !== "") {
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "JSON",
                    url: base_url + "/api/alumno/cambiarEstatusAlumno",
                })
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $.get(base_url+`/api/alumno/conceptosBaja`, function(data) {
