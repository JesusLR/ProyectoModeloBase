...
<html>
  ...
  <body>
    @php
      ...
      $parciales = [
        '1P' => 'inscCalificacionParcial1',
        '2P' => 'inscCalificacionParcial2',
        '3P' => 'inscCalificacionParcial3',
        'ORD' => 'inscCalificacionOrdinario',
        'FINAL' => 'incsCalificacionFinal',
      ];
      # dependiendo del parcial, devuelve el campo correspondiente
      $parciales_faltas = [
        '1P' => 'inscFaltasParcial1',
        '2P' => 'inscFaltasParcial2',
        '3P' => 'inscFaltasParcial3',
      ];
    @endphp
    <header>
      <div class="row">
        ...
        <div class="columns medium-12">
          <table class="table">
            <tr>
              ...
              @if(in_array($tipoCalificacion, ['1P', '2P', '3P']))
            ...
            @foreach ($cgt->inscritos as $inscrito)
              <tr>
                ...
                <td style="width: 120px; font-size: 10px;">
                  {{$inscrito->curso->alumno->persona->nombreCompleto(true)}}
                </td>
                @php
                $calificaciones = [];
                @endphp
                @foreach ($cgt->materias as $materia)
                  ...
                  <td style="" align="right">
                    @if($calif)
                      @if (
                        ...
                        @endphp
                        <span>{{ ($calif->{$parciales[$tipoCalificacion]}) }}</span>
                        @endif
                      @else
                        ...
                        array_push($calificaciones, $calif->{$parciales[$tipoCalificacion]});
                        @endphp
                          {{ ($calif->{$parciales[$tipoCalificacion]}) }}
                        @endif
                      @endif
                    @endif
                  </td>
                  ...
                  @if ($incluyeFaltas && ($tipoCalificacion != "ORD" && $tipoCalificacion != "FINAL"))
                    <td style="">
                      @if ($calif && $calif->{$parciales_faltas[$tipoCalificacion]} > 0)
                        {{ ($calif->{$parciales_faltas[$tipoCalificacion]}) }}
                      @endif
                    </td>
                  @endif
                @endforeach
                <td style="width: 5px;" align="right">
                  @php
                    ...
                    $promedio = 0;
                    $sumaCalificaciones = $materiasInscrito->sum($parciales[$tipoCalificacion]);
                    $sumaFaltas = in_array($tipoCalificacion, ['1P', '2P', '3P']) ? $materiasInscrito->sum($parciales_faltas[$tipoCalificacion]) : 0;
                    $promedio = $sumaCalificaciones > 0 && count($calificaciones) > 0 ? round($sumaCalificaciones / count($calificaciones), 3) : 0;
                    $promedioTotal += $promedio;
                  @endphp
                  {{number_format((float)($promedio), 3)}}
                </td>
                @if(in_array($tipoCalificacion, ['1P', '2P', '3P']))
                  <td style="width: 5px;">
                    <span>
                      {{$sumaFaltas > 0 ? $sumaFaltas : ''}}
                    </span>
                  </td>
                @endif
                <td>
            ...
            <tr>
              ...
              @foreach ($cgt->materias as $materia)
                @php
                  ...
                  $totalInscritos += $grupoInscritos->count();
                @endphp

                <td style="" align="right">
                  {{$materia->esNumerica() ? $promedioGrupo : ''}}
                </td>
                @if ($incluyeFaltas && ($tipoCalificacion != "ORD" && $tipoCalificacion != "FINAL"))
                  <td style=""></td>
                @endif
              @endforeach
              <td style="width: 5px;" align="right">{{$promedioTotal > 0 && $grupoInscritos->count() > 0 ? round($promedioTotal/$grupoInscritos->count(),3) : 0}}</td>
