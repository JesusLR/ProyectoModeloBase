...
<html>
  ...
  <body>
    {{-- Controlador: CargaGruposMaestroController.php --}}
    <header>
      <div class="row">
        <div class="columns medium-6">
          <h5 style="margin-top:0px; margin-bottom: 10px;">UNIVERSIDAD MODELO</h5>
        <h5 style="margin-top:0px; margin-bottom: 10px;">Carga de Grupos por Maestro {{$tipoReporte == true ? '(Detalle)' : '(Resumen)'}}</h5>
        </div>
        <div class="columns medium-6">
      ...
      <div class="columns medium-6">
        <p>{{$periodo}}</p>
        <p>Las horas docentes son solo de la ubicación seleccionada</p>
      </div>
        <div class="columns medium-6">
    ...
    $sumaHrsContrato = 0;
    $sumaHorasDocentes = 0;
    $sumaHorasNoDocentes = 0;
    $sumaHorasTotales = 0;
    @endphp
    @foreach ($datos as $empleado)
      @php
          $maestro = $empleado->first();
      @endphp
      <tbody>
       <tr>
        ...
        <td style="width: 40px;" align="center">{{$maestro['hrsContrato']}}</td>
        <td style="width: 40px;" align="center">{{$maestro['totalHorasDocentes']}}</td>
        <td style="width: 35px;" align="center">{{$maestro['hrsNoDoc']}}</td>
        <td style="width: 20px;" align="center">{{$maestro['totalHoras']}}</td>
        <td style="width: 35px;" align="center">{{$maestro['gruposTotales'] }}</td>
        <td style="width: 20px;" align="center">{{$maestro['alumnosTotales'] }}</td>
      </tr>

      @php
      $sumaHrsContrato += $maestro['hrsContrato'];
      $sumaHorasDocentes += $maestro['totalHorasDocentes'];
      $sumaHorasNoDocentes += $maestro['hrsNoDoc'];
      $sumaHorasTotales += $maestro['totalHoras'];
      @endphp
            @if($tipoReporte == true)
                  ...
                  <tr>
                    <td style="width: 400px;">
                      ...
                      <span style="font-weight: 400; display:inline-block; width: 30px;">
                        ...
                        @endif
                        {{isset($materia['progClave']) ? $materia['progClave']: ""}}
                      </span>
                      <span style="font-weight: 400; display:inline-block; width: 30px;">
                        @if ($loop->first)
                          <div style="margin-top: 5px;"> Plan </div>
                        @endif
                        {{isset($materia['planClave']) ? $materia['planClave']: ""}}

                      </span>
                      <span style="font-weight: 400; display:inline-block; width: 60px;">
                        @if ($loop->first)
                          <div style="margin-top: 5px;"> Materia </div>
                        @endif
                        {{isset($materia['matClave']) ? $materia['matClave']: ""}}
                      </span>
                      <span style="font-weight: 400; display:inline-block; width: 30px;">
                        @if ($loop->first)
                          <div style="margin-top: 5px;"> Gpo </div>
                        @endif
                        {{isset($materia['gpoClave']) ? $materia['gpoClave']: ""}}
                      ...
                      <span style="font-weight: 400; display:inline-block; width: 30px;">
                        ...
                        @endif
                        {{isset($materia['hrsGrupo']) ? $materia['hrsGrupo'] ? $materia['hrsGrupo'] : 0: 0}}
                      ...
                      <span style="font-weight: 400; display:inline-block; width: 30px;">
                        ...
                        @endif
                        {{isset($materia['inscritos_gpo']) ? $materia['inscritos_gpo'] ? $materia['inscritos_gpo'] : 0: 0}}
            ...
            <tr>
              ...
              <td style="width: 40px;" align="center">{{$sumaHrsContrato}}</td>
              <td style="width: 40px;" align="center">{{$sumaHorasDocentes}}</td>
              <td style="width: 40px;" align="center">{{$sumaHorasNoDocentes}}</td>
              <td style="width: 40px;" align="center">{{$sumaHorasTotales}}</td>
              <td style="width: 40px;" align="center"></td>
              <td style="width: 40px;" align="center"></td>
            </tr>
            @endif
