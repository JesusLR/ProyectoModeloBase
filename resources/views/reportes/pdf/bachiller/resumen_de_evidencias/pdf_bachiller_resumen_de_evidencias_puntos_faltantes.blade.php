...
<html>
  ...
  <body>
    ...
    @php
      ...
      $numeroAdasSumaProcesoACD = 0;
      $numeroAdasSumaProduccionACD = 0;

      $puntosTotalesCapturados = 0;
    @endphp
            ...
            <table class="table table-bordered">
                <thead>
                  <tr>
                    ...
                    <th style="width: 50px;" align="center">Ptos Capt</th>
                    ...
                    @foreach ($valores as $llave => $values)
                      @if ($matClave == $values->matClave && $pos++ == 1)
                        @php
                          ...
                          bachiller_materias.bachiller_matPorcentajeCalificacion
                          ...
                          foreach($evidencias as $evi){
                            ...
                            $puntosTotalesCapturados = $puntosTotalesCapturados + $numeroAdasSumaProceso + $numeroAdasSumaProduccion;
                          }

                          $evidencias_collect = collect($evidencias);
                        @endphp

                        @if (number_format((float)$numeroAdasSumaProceso+$numeroAdasSumaProduccion, 1, '.', '') < number_format((float)$evidencias_collect[0]->bachiller_matPorcentajeCalificacion, 1, '.', ''))
                        <tr>
                          ...
                          <td>
                            {{$evidencias_collect[0]->matClave}}
                          </td>
                        
                          <td>
                            {{$evidencias_collect[0]->matNombre}}
                          </td>

                          <td>
                            {{$evidencias_collect[0]->gpoMatComplementaria}}
                          </td>
                        
                          <td align="center">
                            {{$evidencias_collect[0]->matSemestre}}
                          </td>
                          ...
                          <th align="center">
                            {{number_format((float)$evidencias_collect[0]->bachiller_matPorcentajeCalificacion, 1, '.', '')  }}
                          </th>
                        </tr>
                        @endif
                ...
                <tbody>
                  @foreach ($materiasAcdTotalesId as $itemIdAcd => $values)
                    @foreach ($values as $item2)
                      @if ($itemIdAcd == $item2->bachiller_materia_acd_id && $pos3++ == 1)
                      @php
                          $evidenciasACD =  DB::select("SELECT 
                          bachiller_evidencias.id,
                          bachiller_evidencias.periodo_id,
                          bachiller_evidencias.bachiller_materia_id,
                          bachiller_evidencias.eviNumero,
                          ...
                          INNER JOIN escuelas AS escuelas ON escuelas.id = programas.escuela_id
                          LEFT JOIN bachiller_materias_acd AS bachiller_materias_acd ON bachiller_materias_acd.id = bachiller_evidencias.bachiller_materia_acd_id
                          WHERE bachiller_evidencias.periodo_id = $item2->periodo_id
                          AND programas.id = $item2->programa_id
                          AND planes.id = $item2->plan_id
                          AND bachiller_materias.matSemestre = $item2->matSemestre
                          AND bachiller_evidencias.bachiller_materia_id = $item2->bachiller_materia_id
                          AND bachiller_evidencias.bachiller_materia_acd_id = $itemIdAcd
                          AND bachiller_evidencias.deleted_at IS NULL
                          AND bachiller_materias.deleted_at IS NULL
                          AND periodos.deleted_at IS NULL
                          AND departamentos.deleted_at IS NULL
                          ...
                          foreach($evidenciasACD as $eviACD){

                            if($item2->bachiller_materia_id == $eviACD->bachiller_materia_id && $itemIdAcd == $eviACD->bachiller_materia_acd_id){
                              $numeroAdasACD++;
                            }
                            ...
                            #puntos proceso
                            if($eviACD->eviTipo == "A"){
                              $numeroAdasSumaProcesoACD = $numeroAdasSumaProcesoACD + $eviACD->puntosMaximos; 
                            }

                            #puntos producción
                            if($eviACD->eviTipo == "P"){
                              $numeroAdasSumaProduccionACD = $numeroAdasSumaProduccionACD + $eviACD->puntosMaximos; 
                            }
                          }

                          $evidencias_collect = collect($evidenciasACD);
                        @endphp

                        @if (number_format((float)$numeroAdasSumaProcesoACD+$numeroAdasSumaProduccionACD, 1, '.', '') < number_format((float)$evidencias_collect[0]->bachiller_matPorcentajeCalificacion, 1, '.', ''))
                        <tr>
                          <td align="center">{{$pos2++}}</td>
                          <td>{{$evidenciasACD[0]->matClave}}</td>
                          <td>{{$evidenciasACD[0]->matNombre}}</td>
                          <td>{{$evidenciasACD[0]->gpoMatComplementaria}}</td>
                          <td align="center">{{$evidenciasACD[0]->matSemestre}}</td>
                          <td align="center">{{$numeroAdasACD}}</td>
                          <td align="center">{{$numeroAdasSumaProcesoACD}}</td>
                          <td align="center">{{$numeroAdasSumaProduccionACD}}</td>

                          <th align="center">
                            {{number_format((float)$numeroAdasSumaProcesoACD+$numeroAdasSumaProduccionACD, 1, '.', '')  }}
                          </th>
