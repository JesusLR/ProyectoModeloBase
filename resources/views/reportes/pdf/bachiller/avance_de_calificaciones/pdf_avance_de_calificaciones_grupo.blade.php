    ...
    @foreach ($valores as $valorAgrupado)
      ...
      <div class="row">
        <div class="columns medium-3">
          <p><b>Clave:</b> {{$valorAgrupado->clave_pago}}</p>
      ...
      <div class="row">
        <div class="columns medium-12">
          <table class="table table-bordered">
              ...
              @foreach ($materias_alumno as $matClave => $valoresObtenidos)
                @foreach ($valoresObtenidos as $item)
                  @if ($matClave == $item->matClave && $item->clave_pago == $valorAgrupado->clave_pago && $pos++ == 1)
                    ...
                    @endphp
                      <tr>
                        <td style="width: 15px; border-top: 1px solid; border-right: 0px solid; border-bottom: 1px solid; border-left: 1px solid;">{{$item->matClave}}</td>
            ...
            {{--  materias acd   --}}
            <tbody>
              @foreach ($alumno_acd as $item_aluClave => $alumno_acd_val)
                @foreach ($alumno_acd_val as $item10)
                  ...
                  @php
                    ...
                    INNER JOIN bachiller_evidencias AS bachiller_evidencias ON bachiller_evidencias.id = bachiller_inscritos_evidencias.evidencia_id
                    INNER JOIN bachiller_materias AS bachiller_materias ON bachiller_materias.id = bachiller_evidencias.bachiller_materia_id
                    WHERE bachiller_inscrito_id=$item10->bachiller_inscrito_id
                    AND bachiller_evidencias.periodo_id = $item10->periodo_id
                    AND bachiller_evidencias.bachiller_materia_id = $item10->bachiller_materia_id
                    AND bachiller_evidencias.bachiller_materia_acd_id = $item10->bachiller_materia_acd_id
                    AND bachiller_evidencias.deleted_at IS NULL
                    AND bachiller_inscritos_evidencias.deleted_at IS NULL
                    AND bachiller_materias.deleted_at IS NULL");
                    ...
                    FROM bachiller_evidencias AS bachiller_evidencias
                    INNER JOIN bachiller_materias AS bachiller_materias ON bachiller_materias.id = bachiller_evidencias.bachiller_materia_id
                    WHERE bachiller_evidencias.periodo_id = $item10->periodo_id
                    AND bachiller_evidencias.bachiller_materia_id = $item10->bachiller_materia_id
                    AND bachiller_evidencias.bachiller_materia_acd_id = $item10->bachiller_materia_acd_id
                    AND bachiller_evidencias.deleted_at IS NULL
                    AND bachiller_materias.deleted_at IS NULL");
                ...
                @endphp
                  <tr>
                    <td style="width: 15px; border-top: 1px solid; border-right: 0px solid; border-bottom: 1px solid; border-left: 1px solid;">{{$item10->matClave}}</td>
                    <td style="width: 270px; border-top: 1px solid; border-right: 0px solid; border-bottom: 1px solid; border-left: 1px solid;">{{$item10->matNombre}}-<b>{{$item10->gpoMatComplementaria}}</b></td>
                    <td style="border-top: 1px solid; border-right: 0px solid; border-bottom: 1px solid; border-left: 0px solid;" align="center"></td>
                    <td align="center">
      ...
      <div class="row">
        <div class="columns medium-12">
          <table class="table table-bordered">
            ...
            <tbody>
              @php
                $aluClave = $valorAgrupado->clave_pago;
                $periodo_id = $valorAgrupado->periodo_id;
