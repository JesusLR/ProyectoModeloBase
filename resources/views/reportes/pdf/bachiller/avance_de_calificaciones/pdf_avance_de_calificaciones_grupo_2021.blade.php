            ...
            <tbody>
              @foreach ($materias_alumno as $matClave => $valoresObtenidos)
                @foreach ($valoresObtenidos as $item)
                  @if ($item->clave_pago == $valorAgrupado->clave_pago  && $pos++ == 1)
                    @php
                    ...
                    @endphp
                    <tr>
                      <td style="width: 15px; border-top: 1px solid; border-right: 0px solid; border-bottom: 1px solid; border-left: 1px solid;">{{$matClave}}</td>
