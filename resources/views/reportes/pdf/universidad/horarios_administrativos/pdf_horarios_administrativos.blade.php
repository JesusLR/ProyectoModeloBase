  ...
  <body>
    ...
    <div class="row">
      <div class="columns medium-12">
        ...
        @php
          $empleado = Empleado::select('personas.*', 'escuelas.escClave')
        ...
        <p style="font-size: 13px;"><b>{{ $item->empleado_id }} - {{ $empleado->perApellido1.' '.$empleado->perApellido2.' '.$empleado->perNombre }} - Escuela: {{ $empleado->escClave }}</b></p>
        <br>
        <table class="table">
          ...
          @foreach ($agrupamosgrupos as $grupo_id => $los_valores)
            @foreach ($los_valores as $gro)
              @if ($grupo_id == $gro->grupo_id && $contador++ == 1)
                ...
                <p><b>{{ $gro->gpoSemestre }}{{ $gro->gpoClave }} - {{ $gro->matNombre }}</b></p>
