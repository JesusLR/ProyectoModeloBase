    ...
    <header>
      ...
      <div class="row">
        <div class="columns medium-12">
          <p style="font-size: 15px; margin-bottom: 15px; text-align: center; margin-bottom: 50px;">No: {{$minutario}}</p>
          <p style="font-size: 15px; margin-bottom: 15px;">A QUIEN CORRESPONDA:</p>
          <p style="font-size: 15px; margin-bottom: 35px; text-align: justify;">
            {{$firmante->firSexo == "M"? "El": ""}}
            {{$firmante->firSexo == "F"? "La": ""}}
            que suscribe,
            {{$firmante->firNombre}},
            {{$firmante->firPuesto}}
            de la {{$departamento->depNombreOficial}}
            con clave de incorporación {{$departamento->depClaveOficial}} hace constar que:
          </p>
          <p style="font-size: 15px; margin-bottom: 35px; text-align: justify;">
            {{$persona->perSexo == "M" ? "El": ""}}
            {{$persona->perSexo == "F" ? "La": ""}}
            joven {{MetodosPersonas::nombreCompleto($persona)}},
            con clave {{$alumno->aluClave}},
            {{($periodo->id == $departamento->perActual || $periodo->id == $departamento->perSig) ? "es": "fue"}}
            ...
            @endif
            semestre de
            {{$curso->cgt->plan->programa->progNombre}} en el ciclo escolar del
            ...
            {{$periodo->departamento->perActual == $periodo->id ? "y se encuentra cursando": "cursando"}}
          ...
          <div style="margin-bottom: 35px;">
            @foreach ($materias as $materia)
              @php
                $nombre_materia = $materia->grupo->materia->matNombre;
                if($materia->grupo->optativa)
                  $nombre_materia .= " - {$materia->grupo->optativa->optNombre}";
              @endphp
              <p style="font-size: 15px;  text-align: justify;">{{$nombre_materia}}</p>
            @endforeach
          </div>
          ...
          <p style="font-size: 15px; text-align: center;">{{$firmante->firPuesto}}</p>
        </div>
