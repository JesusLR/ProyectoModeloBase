...
<head>
    <title>Lista de asistencia ACD</title>
...
@endphp
<body>
    @foreach ($primaria_grupo_id as $primaria_grupo => $alumnosInscritos)

    <div class="row">
        <div class="columns medium-12">
            <label style="margin-top:0px; margin-bottom: 10px;">ESCUELA SEC. PART. INC. MODELO</label>
            <br>
            ...
            @foreach ($inscritos as $key => $itemInscritos)
                @if ($primaria_grupo == $itemInscritos->primaria_grupo_id && $vuelta2++ == 1)
                <p>Materia: {{$itemInscritos->gpoMatComplementaria}} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Grado
                    : {{$itemInscritos->cgtGrado}}° Grupo: {{$itemInscritos->gpoClave}}
                </p>
                <p>
                    Docente: {{$itemInscritos->docente_ape_paterno}} {{$itemInscritos->docente_ape_materno}} {{$itemInscritos->docente_nombre}} ({{$itemInscritos->empleado_id_docente}})
                </p>
                @endif
            @endforeach
            <p></p>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th align="center" style="font-weight: 400;width: 210px; padding-top: 7px;">Nombre del Alumno
                        </th>
                        <th align="center" style="font-weight: 400;width: 20px; padding-top: 7px;">Gdo</th>
                        <th align="center" style="font-weight: 400;width: 20px; padding-top: 7px;">Gpo</th>

                        @for ($i = 0; $i < 25; $i++) <th style="font-weight: 400;">
                            |__|
                            </th>
                            @endfor
                ...
                <tbody>
                        @foreach ($inscritos as $key => $itemInscritos)
                            @if ($primaria_grupo == $itemInscritos->primaria_grupo_id)
                            <tr>

                                <td align="center" style="padding-top: 5px; width: 20px;">{{$total++}}</td>
                                <td style="padding-top: 5px;">{{$itemInscritos->clavePago}}</td>
                                <td style="padding-top: 5px;">{{$itemInscritos->ape_paterno}} {{$itemInscritos->ape_materno}}
                                    {{$itemInscritos->nombres}}</td>
