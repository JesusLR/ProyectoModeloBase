...
<html>
  ...
  @foreach ($curso_alumno as $curso)
    @php
      ...
      if ($curso->cgtGradoSemestre == 2) {
        $gradoEnLetras = "SEGUNDO GRADO";
      }
      if ($curso->cgtGradoSemestre == 3) {
        $gradoEnLetras = "TERCER GRADO";
      }
      if ($curso->cgtGradoSemestre == 4) {
        $gradoEnLetras = "CUARTO GRADO";
      }
      if ($curso->cgtGradoSemestre == 5) {
      ...
      if(isset($resultado_array[0]->gpoClave)){
        $gpoClave = $resultado_array[0]->gpoClave;
      }else{
        $gpoClave = "";
      }
    @endphp
    ...
    <body>
      ...
      <div class="row">
        <div class="columns medium-12">
          ...
          <br>
          <p class="tright">ASUNTO: CONSTANCIA DE ESTUDIOS</p>
          <br>
          <br>
          <br>
          <br>
          ...
          @if ($parametro_ubicacion_clave == "CME")
            <p style="text-indent: 3em; font-size: 15px; text-align: justify;">
              ...
              Escuela Secundaria "Modelo", clave C.T. 31PES0012T de esta ciudad hace constar:
            </p>
          @endif

          {{--  @if ($parametro_ubicacion_clave == "CVA")
            <p style="text-indent: 3em; font-size: 15px; text-align: justify;">
              La que suscribe,  Mtra. Lol –Há Canché Gómez, Directora académica de la
          ...
          <p style="text-indent: 3em; font-size: 15px; text-align: justify;">
            {{$parametro_genero_alumno}} <b>{{$curso->perApellido1.' '.$curso->perApellido2.' '.$curso->perNombre}} con número de matrícula {{$curso->aluClave}} </b>  {{$parametro_consideracion}} regular de este plantel, al {{$gradoEnLetras}} GRUPO “{{$gpoClave}}, 
