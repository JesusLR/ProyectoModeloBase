...
class ActaExtraordinarioController extends Controller
{
  ...
  public function reporte()
  {
    ...
    return View('reportes-federal.acta_extraordinario.create',compact('ubicaciones'));
  }
  ...
  public function imprimir(Request $request)
  {
    $inscritosEx = InscritoExtraordinario::with('extraordinario.materia.plan.programa.escuela.departamento.ubicacion', 'alumno.persona','extraordinario.periodo')
      ->whereHas('extraordinario.materia.plan', static function($query) use ($request) {
    ...
    foreach($inscritosEx as $inscrito){
      ...
      //Datos del alumno
      $aluMatricula = $inscrito->alumno->aluMatricula;
      ...
      if($inscrito->extraordinario->materia->esAlfabetica()) {
        if (!is_null($iexCalificacion)) {
          ...
          $iexCalificacion = $iexCalificacion == 0 ? 'APROBADO' : 'NO APROBADO';
        }
      }
      ...
      $inscritoEx->push([  
        ...
        'iexCalificacion'=>$this->getCalificacion($inscrito->extraordinario->materia, $iexCalificacion),
    ...
    return PDF::loadView('reportes.pdf.federal.'. $nombreArchivo, [
  ...
  public function getCalificacion($materia, $calificacion)
	{
