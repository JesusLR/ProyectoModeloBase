...
class SecundariaCalificacionPorGrupoController extends Controller
{
    ...
    public function Reporte()
    {
        ...
        return view('secundaria.reportes.calificaciones_por_grupo.boleta-calificaciones-create', [
    ...
    public function imprimirCalificaciones(Request $request)
    {
        ...
        if($tipoReporte == "porMes"){
            $parametro_Titulo = "LISTA PARA REPORTAR CALIF. MENSUAL";
        }
        if($tipoReporte == "porBimestre"){
            $parametro_Titulo = "LISTA PARA REPORTAR CALIF. BIMESTRAL";
        }
        if($tipoReporte == "porTrimestre"){
            $parametro_Titulo = "LISTA PARA REPORTAR CALIF. TRIMESTRAL";
        }
        ...
        $parametro_NombreArchivo = 'pdf_secundaria_calificaciones'; //nombre del archivo blade
        ...
        if($tipoCalificacionVista == "todosGrupos"){
            ...
            // view('reportes.pdf.secundaria.calificacion_por_grupo.pdf_secundaria_calificaciones');
            $pdf = PDF::loadView('reportes.pdf.secundaria.calificacion_por_grupo.' . $parametro_NombreArchivo, [
            ...
            $parametro_NombreArchivo = 'pdf_secundaria_calificaciones_sep'; //nombre del archivo blade
        ...
        // dsdsfsdsdsdsdsdsdsdsdsdsdsdsdsdsd

        // $pdf = PDF::loadView('reportes.pdf.secundaria.calificacion_por_mes_grupo.' . $parametro_NombreArchivo, [
        //     "materia_alumos" => $materia_alumos,
        //     "calificaciones" => $resultado_collection,
        //     "fechaActual" => $fechaActual->format('d/m/Y'),
        //     "horaActual" => $fechaActual->format('H:i:s'),
    ...
    //     ->select('alumnos.aluClave', DB::raw('count(*) as aluClave, alumnos.aluClave'),
    //     'personas.perApellido1', DB::raw('count(*) as perApellido1, personas.perApellido1'),
    //     'personas.perApellido2', DB::raw('count(*) as perApellido2, personas.perApellido2'),
    //     'personas.perNombre', DB::raw('count(*) as perNombre, personas.perNombre'),
    //     'alumnos.aluClave', DB::raw('count(*) as aluClave, alumnos.aluClave'),
    //     'alumnos.id', DB::raw('count(*) as id, alumnos.id'))
    //     ->leftJoin('secundaria_grupos', 'secundaria_inscritos.grupo_id', '=', 'secundaria_grupos.id')
    //     ->leftJoin('secundaria_materias', 'secundaria_grupos.secundaria_materia_id', '=', 'secundaria_materias.id')
    //     ->leftJoin('planes', 'secundaria_grupos.plan_id', '=', 'planes.id')
    //     ->leftJoin('periodos', 'secundaria_grupos.periodo_id', '=', 'periodos.id')
    ...
    //     $parametro_Titulo = "HOJA PARA REPORTAR CALIFICACIONES POR GRUPO";
    ...
    //             "trimestreEvaluar" => $trimestreEvaluar,
    //             "tipoCalificacionVista" => $tipoCalificacionVista,
    //             "resultadoMatSep_collection" => $resultadoMatSep_collection
    //         ]);
    ...
    //             "tipoCalificacionVista" => $tipoCalificacionVista,
    //             "matNombreColumna" => $matNombreColumna,
    //             "calif" => $resultadoMatSep_collection,
    //             "matClave" => $matClave
    //         ]);
