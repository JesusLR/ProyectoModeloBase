...
class SecundariaExpedienteAlumnosController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('secundaria.reportes.expediente_alumnos.create', [
    ...
    public function imprimirExpediente(Request $request)
    {
        ...
        if($tipoReporte == 3){
            $parametro_NombreArchivo = "pdf_secundaria_expediente_blanco_nuevo";
            ...
            if($aluClave != ""){
                ...
                $unicoAlumno =  DB::select("call procSecundariaUnAlumnoInscritoGrupo(". $periodo_id . "," . $grado . ",'" . $grupo . "','" . $concepto . "', ".$alumno->id.")");
                ...
                $unicoAlumnoHistoria =  DB::select("call procSecundariaDatosHistoria(".$alumno->id.")");
                // return $alumnoHistoria_collection = collect($unicoAlumnoHistoria);
                $alumnoHistoria_collection = collect($unicoAlumnoHistoria);

    
                // Obtener tutores 
                $unicoAlumnoTutores =  DB::select("call procSecundariaDatosTutores(".$alumno->id.")");
                ...
                $parametro_NombreArchivo = "pdf_secundaria_expediente_uno_nuevo";
                $pdf = PDF::loadView('reportes.pdf.secundaria.expediente.' . $parametro_NombreArchivo, [
