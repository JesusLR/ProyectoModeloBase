...
class SecundariaAplicarPagosController extends Controller
{
  /**
  ...
  {
    return View('secundaria.aplicar_pagos.show-list');
  }
      ...
      ->addColumn('action', function($query) {
        ...
        if($query->pagFormaAplico == "M" || in_array(auth()->user()->permiso('registro_cuotas'), ['A', 'B'])) {
          $btn_editar = '<div class="col s1">
                              <a href="/secundaria/pagos/aplicar_pagos/edit/' . $query->id . '" class="button button--icon js-button js-ripple-effect" title="Editar">
                                  <i class="material-icons">edit</i>
                              </a>
                        </div>';
          $btn_borrar = '<div class="col s1">
                            <form id="delete_' . $query->id . '" action="/secundaria/pagos/aplicar_pagos/delete/' . $query->id . '" method="POST">
        ...
        return '<div class="row">'
                    .Utils::btn_show($query->id, '/secundaria/pagos/aplicar_pagos/detalle')
                    .$btn_editar
                    .$btn_borrar.
               '</div>';
      })
  ...
  public function create(Request $request)
  {
    ...
    return View('secundaria.aplicar_pagos.create', [
  ...
  public function edit(Request $request)
  {
    ...
    return View('secundaria.aplicar_pagos.edit', [
      "conceptosPago" => $conceptosPago,
      "pago" => $pago
    ]);
  }
  ...
  public function detalle(Request $request)
  {
    ...
    return view("secundaria.aplicar_pagos.detalle", [
      "pago" => $pago,
      "cursos" => $cursos,
      "usuario" => $usuario,
