...
class PreescolarAlumnosBecadosController extends Controller
{
    ...
    public function reporte()
    {
      ...
      $ubicaciones = Ubicacion::whereIn('id', [1])->sedes()->get();

      return view('preescolar.reportes.alumnos_becados.create', [
        'tiposBeca' => $tiposBeca,
    ...
    public function imprimir(Request $request)
    {
      ...
      // view('reportes.pdf.preescolar.alumnos_becados.pdf_alumnos_becados')
      return PDF::loadView('reportes.pdf.preescolar.alumnos_becados.' . $nombreArchivo, [
        "datos" => $this->alumnos,
    ...
    */
    ...
    {
      return Secundaria_inscritos::with('curso')
      ->whereHas('curso', static function($query) use ($cursos_ids) {
        $query->whereIn('curso_id', $cursos_ids);
      })
      ->whereHas('secundaria_grupo.secundaria_materia', static function($query) {
        $query->where('matTipoAcreditacion', 'N');
      })->get()->pluck('incsPromedioMes', 'secundaria_inscrito.curso.alumno_id');
    }
