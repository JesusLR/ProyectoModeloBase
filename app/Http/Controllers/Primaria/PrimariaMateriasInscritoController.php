...
class PrimariaMateriasInscritoController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('primaria.cargarMateriasInscrito.create', [
            "alumno" => $alumno,
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            if($cgt_id != ""){
                ...
                $resultado_array =  DB::select("call procPrimariaAgregaGruposNuevosInscritos(" . $curso_id . ",  " . $cgt_id . ")");
        ...
        if($request->ajax()){
            $alumnos = Alumno::with("persona")
                ...
                ->whereHas('persona', function($query) use ($request) {
                    $query->whereRaw("CONCAT(perNombre, ' ', perApellido1) like ?", ["%{$request->nombreAlumno}%"]);
                });
