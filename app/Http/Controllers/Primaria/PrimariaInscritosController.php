...
class PrimariaInscritosController extends Controller
{
    ...
    public function list($grupo_id)
    {
        ...
        // perAnioPAgo 
        ->filterColumn('modalidad_asistencia',function($query,$keyword){
            $query->whereRaw("CONCAT(inscTipoAsistencia) like ?", ["%{$keyword}%"]);
        })
        ->addColumn('modalidad_asistencia',function($query){
            return $query->inscTipoAsistencia;
        })

        ->filterColumn('docente_asignado',function($query,$keyword) {
            $query->whereRaw("CONCAT(empNombre, ' ', empApellido1, ' ', empApellido2) like ?", ["%{$keyword}%"]);
        })
        ->addColumn('docente_asignado',function($query) {
