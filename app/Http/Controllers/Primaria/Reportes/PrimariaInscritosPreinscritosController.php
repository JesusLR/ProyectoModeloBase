...
class PrimariaInscritosPreinscritosController extends Controller
{
     /**
    ...
    public function reporte()
    {
        ...
        $departamentos = Departamento::where('depClave', '=', 'PRI')->get();

        //dd($departamentos);

        return view('primaria.reportes.inscritos_preinscritos.create',compact('tiposIngreso',
    ...
    public function imprimir(Request $request)
    {
        ...
        if (count($cgts)) {
            ...
            $pdf->AliasNbPages();
            $pdf->SetTitle('Relacion de Primaria Inscritos');
            $pdf->SetFont('Times', '', 9);
...
class PDF extends Fpdf
{
    function Header()
    {
        ...
        $this->Ln($altoTitulo);
        $this->Cell(0, $altoTitulo, "PrimariaInscritosPreinscritosController.php", $dibujarLinea, 0, 'R');
        $this->Ln($altoTitulo);
