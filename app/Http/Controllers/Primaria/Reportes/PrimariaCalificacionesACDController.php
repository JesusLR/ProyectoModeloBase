...
class PrimariaCalificacionesACDController extends Controller
{
    ...
    public function reporteBoleta()
    {
        ...
        return view('primaria.reportes.boleta_de_calificaciones_ACD.create', [
    ...
    public function imprimir(Request $request)
    {
        ...
        if ($request->gpoClave != "") {
            ...
            $resultado_array =  DB::select("call procPrimariaCalificacionesACDGradoGrupo(" . $request->programa_id . ", 
            " . $request->plan_id . ",
            " . $request->periodo_id . ",
            " . $request->gpoGrado . ",
            '" . $request->gpoClave . "')");
        ...
        if ($request->gpoClave == "" && $request->aluClave == "") {
            ...
            $resultado_array =  DB::select("call procPrimariaCalificacionesACDGradoCompleto(" . $request->programa_id . ", 
                    " . $request->plan_id . ",
                    " . $request->periodo_id . ",
                    " . $request->gpoGrado . ")");
            $resultado_collection = collect($resultado_array);
        ...
        if ($request->aluClave != "") {
            $resultado_array =  DB::select("call procPrimariaCalificacionesACDAlumno(" . $request->programa_id . ", 
                    " . $request->plan_id . ",
                    " . $request->periodo_id . ",
                    " . $request->gpoGrado . ",
                    " . $request->aluClave . ")");
        ...
        $parametro_NombreArchivo = 'pdf_primaria_boleta_calificaciones_general_acd';
        $pdf = PDF::loadView('reportes.pdf.primaria.boleta_de_calificaciones_acd.' . $parametro_NombreArchivo, [
    ...
    public function boletadesdecurso($curso_id)
    {
        ...
        $parametro_NombreArchivo = 'pdf_primaria_boleta_calificaciones_curso';
        $parametro_Titulo = "BOLETA DE CALIFICACIONES DEL ALUMNO(A)";
        $resultado_array =  DB::select("call procPrimariaCalificacionesACDCurso("
            .$curso_id
            .")");
        $resultado_collection = collect( $resultado_array );
