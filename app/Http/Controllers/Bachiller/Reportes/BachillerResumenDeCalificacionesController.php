...
class BachillerResumenDeCalificacionesController extends Controller
{
    ...
    public function reporteResumenCalificacion(Request $request)
    {
        ...
        // Parametro
        $parametro_ubicacion_clave = $datos_cabecera[0]->ubiClave;

        if ($tipoCalificacionVista == "todasLasMaterias") {

            $totalDeAlumnos = count($calificacionesInscritos);
        ...
        if ($tipoCalificacionVista == "matOficialesSep") {
            ...
            if ($parametro_ubicacion_clave == 'CME'){
                if($tipoReporte == "porTrimestre"){
                    $parametro_NombreArchivo = 'pdf_secundaria_resumen_de_calificaciones_tri_sep'; //nombre del archivo blade    
                }else{
                    $parametro_NombreArchivo = 'pdf_secundaria_resumen_de_calificaciones_sep_cme'; //nombre del archivo blade
                }
            }
