...
class BachillerAlumnosBecadosController extends Controller
{
    ...
    public function reporte()
    {
      ...
      return view('bachiller.reportes.alumnos_becados.create', [
        'tiposBeca' => $tiposBeca,
    ...
    public function imprimir(Request $request)
    {
      ...
      // validamos el campus para llamar el correspondiente 
      if($request->ubicacion_id == 1 || $request->ubicacion_id == 2){
        $promedios = $this->buscar_promedios_yucatan($cursos_ids);
      }else{
        if($request->ubicacion_id == 3){
          $promedios = $this->buscar_promedios_chetumal($cursos_ids);
        }
      }
      ...
      return PDF::loadView('reportes.pdf.bachiller.alumnos_becados.' . $nombreArchivo, [
        "datos" => $this->alumnos,
    ...
    */
  //  para yucatan 
    private function buscar_promedios_yucatan($cursos_ids)
    {
      return Bachiller_inscritos::with('curso')
      ->whereHas('curso', static function($query) use ($cursos_ids) {
        $query->whereIn('curso_id', $cursos_ids);
      })
      ->whereHas('bachiller_grupo.bachiller_materia', static function($query) {
        $query->where('matTipoAcreditacion', 'N');
      })->get()->pluck('incsPromedioMes', 'bachiller_inscrito.curso.alumno_id');
    }

    // para chetumal 
    private function buscar_promedios_chetumal($cursos_ids)
    {
      return Bachiller_cch_inscritos::with('curso')
      ->whereHas('curso', static function($query) use ($cursos_ids) {
        $query->whereIn('curso_id', $cursos_ids);
      })
      ->whereHas('bachiller_cch_grupo.bachiller_materia', static function($query) {
        $query->where('matTipoAcreditacion', 'N');
      })->get()->pluck('incsPromedioMes', 'bachiller_cch_inscrito.curso.alumno_id');
    }
