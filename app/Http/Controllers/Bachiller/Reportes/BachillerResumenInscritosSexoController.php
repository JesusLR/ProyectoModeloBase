...
class BachillerResumenInscritosSexoController extends Controller
{
  ...
  public function reporte()
  {
    ...
    return view('bachiller.reportes.resumen_inscritos_sexo.create', compact('anioActual', 'ubicaciones'));
  }
  ...
  public function imprimir(Request $request)
  {
    ...
    $resultados =  DB::select("call procBachillerResumenInscritosSexo(
      ".$periodo->perNumero.",
      ".$periodo->perAnio.",
      '".$ubicacion->ubiClave."',
      '".$plan->planClave."')");
    ...
    $periodo =  '(' . $periodo->perNumero . '/' . $periodo->perAnio . ')  ' . Utils::fecha_string($fechas->perFechaInicial, true) . ' - ' . Utils::fecha_string($fechas->perFechaFinal, true);

    $fechaActual = Carbon::now('CDT');

    // view('reportes.pdf.bachiller.resumen_inscritos_sexo.pdf_resumen_inscritos_sexo');
    $nombreArchivo = 'pdf_resumen_inscritos_sexo';
    $pdf = PDF::loadView('reportes.pdf.bachiller.resumen_inscritos_sexo.' . $nombreArchivo, [
