...
class BachillerSOCAController extends Controller
{
    ...
    public function reporte()
    {
        ...
        return view('bachiller.reportes.SOCA.create', [
    ...
    public function imprimir(Request $request)
    {
        ...
        $resultado_collection = DB::select('call procBachillerFormatoSOCAAco(?,?,?,?,?,?)',array($perNumero,
        $perAnio,
        $ubiClave,
        $progClave,
        $cgtGradoSemestre,
    ...
    public function generarExcel($registroAlumnos) {
        ...
        foreach($registroAlumnos as $alumno) {    
            ...
            $sheet->setCellValueExplicit("I{$fila}", $alumno->claveObl1, DataType::TYPE_STRING);
            $sheet->setCellValueExplicit("J{$fila}", $alumno->claveObl2, DataType::TYPE_STRING);
            $sheet->setCellValueExplicit("K{$fila}", $alumno->claveObl3, DataType::TYPE_STRING);
        ...
        try {
            $writer->save(storage_path("SOCA (Optativas).xlsx"));
        } catch (Exception $e) {
            alert('Ha ocurrido un problema', $e->getMessage(), 'error')->showConfirmButton();
            return back()->withInput();
        }
    
        return response()->download(storage_path("SOCA (Optativas).xlsx"));
      }
}
