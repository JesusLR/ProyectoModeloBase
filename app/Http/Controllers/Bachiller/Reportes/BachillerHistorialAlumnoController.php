...
class BachillerHistorialAlumnoController extends Controller
{
  ...
  public function reporte()
  {
    ...
    $programas = Programa::where('progClave','BAC')->get()->unique('progClave');
    $ubicaciones = Ubicacion::whereIn('id', [1, 2])->get();


    return view('bachiller.reportes.historial_alumno.create',compact('anioActual', 'ubicaciones','programas'));
  }
  ...
  */
  private function buscar_resacas($alumno_id, $plan_id = null) {
    ...
    return Bachiller_resumenacademico::with('plan.programa')
  ...
  */
  ...
  public function imprimir(Request $request)
  {
    ...
    $historial = Bachiller_historico::with(['alumno.persona','bachiller_materia.plan', 'periodo'])
      ...
      ->whereHas('bachiller_materia.plan', function($query) use ($request) {
        if ($request->plan_id) {
    ...
    $grupoNom = Bachiller_grupos::select('gpoGrado','gpoClave')->where('bachiller_materia_id',$historialFirst->bachiller_materia_id)
    ...
    foreach ($historial  as $key => $historico){
      $grupo = Bachiller_grupos::where('bachiller_materia_id','=',$historico->bachiller_materia_id)->where('plan_id','=',$historico->plan_id)->where('periodo_id','=',$historico->periodo_id)->first();
      ...
      $matClave = $historico->bachiller_materia->matClave;
      $matNombre = $historico->bachiller_materia->matNombre;
      $matCreditos = $historico->bachiller_materia->matCreditos;
      ...
      $matTipoAcreditacion = $historico->bachiller_materia->matTipoAcreditacion;
      $depCalMinAprob = $historico->bachiller_materia->plan->programa->escuela->departamento->depCalMinAprob;
    ...
    // view('reportes.pdf..bachiller.certificado_kardex.pdf_historial_alumno');
    return PDF::loadView('reportes.pdf..bachiller.certificado_kardex.'. $nombreArchivo, $infoReporte)->stream($nombreArchivo.'.pdf');
