...
class BachillerMejoresPromediosAnualesController extends Controller
{
    ...
    public function reporte()
    {
      ...
      return view('bachiller.reportes.mejores_promedios.create-anuales',compact('ubicaciones'));
    }
    ...
    public function imprimir(Request $request){
      ...
      $cursos = self::buscarCursos($this->request, $perAnioPago);
      ...
      # --------------------------------------------------------------------------------
      $historicoData = self::buscarHistoricos($cursos_periodo, $plan, $perAnioPago);
      # --------------------------------------------------------------------------------
      ...
      $alumnos = $cursos_periodo->map(function($curso) use ($historicoData, $cursos, $calificacion_minima) {
        ...
        $info['promedio'] = $this->calcularPromedio($historicos, $period->perAnioPago);
        ...
        $info['numExt'] = $historicos->where('histPeriodoAcreditacion', 'EX', 'RE', 'AC')->count();
        $info['numDeb'] = $historicos->whereIn('histPeriodoAcreditacion', ['PN','EX', 'RE', 'AC'])->where('histCalificacion', '<', $calificacion_minima)->count();
    ...
    //   view('reportes.pdf.bachiller.mejores_promedios.pdf_mejores_promedios');

      return $request->formato == 'PDF' ? 
        PDF::loadView('reportes.pdf.bachiller.mejores_promedios.'. $nombreArchivo, $info_reporte)->stream($nombreArchivo.'.pdf') : self::generarExcel($info_reporte);
    ...
    */
    private static function buscarCursos($request, $perAnioPago)
    {
      
      return Curso::with(['cgt', 'alumno.persona', 'periodo'])
    ...
    */
    ...
    {
      return Bachiller_historico::whereIn('alumno_id', $cursos->pluck('alumno_id'))
      ->where('plan_id', $plan->id)
      // ->where('periodo_id', $periodo_id)
      ->whereIn('histPeriodoAcreditacion', ['PN', 'EX', 'RV', 'RE', 'AC'])
      ->whereHas('bachiller_materia', static function($query) {
        $query->where('matTipoAcreditacion', 'N');        
        $query->whereNotIn('matClasificacion', ['X', 'C']); 
      ...
      ->keyBy(static function($historico) {
        return "{$historico->alumno_id}-{$historico->bachiller_materia_id}";
      })->groupBy('alumno_id');
    }
    ...
    */
    private function calcularPromedio($historicos, $perAnioPago)
    {
