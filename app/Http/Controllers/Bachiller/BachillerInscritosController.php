...
class BachillerInscritosController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $bachiller_inscritos = Bachiller_inscritos::where('bachiller_grupo_id', '=', $grupo_id)
        ...
        if($sistemas == 1){
            ...
            if($depCME->perActual == $grupo->periodo_id || $dedCVA->perActual == $grupo->periodo_id){
                $activo = true;
            }else{
                $activo = false;
            }
        ...
        return view('bachiller.inscritos.show-list-inscritos', [
            "grupo" => $grupo,
            "activo" => $activo
        ]);
    ...
    public function pase_de_lista($grupo_id)
    {
        ...
        return view('bachiller.inscritos.paseDelista', [
    ...
    public function obtenerAlumnosPaseLista(Request $request, $grupo_id, $fecha)
    {
        ...
        if ($request->ajax()) {
            ...
            ->whereNull('cursos.deleted_at')
            ->whereNull('alumnos.deleted_at')
            ->whereNull('bachiller_asistencia.deleted_at')
            ->orderBy('personas.perApellido1', 'ASC')
            ->orderBy('personas.perApellido2', 'ASC')
            ->get();
