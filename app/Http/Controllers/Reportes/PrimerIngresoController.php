...
class PrimerIngresoController extends Controller
{
  ...
  public function reporte()
  {
    ...
    return View('primer_ingreso.create',compact('tiposIngreso','alumnos_curso','alumnos_estado','tipo_reporte','orden_reporte','espaciado', 'ubicaciones'));
  }
  ...
  public function imprimir(Request $request)
  {
    ...
    ->whereHas('alumno', function($query) use($request){
      ...
      if ($request->aluEstado)
        $query->where('aluEstado', 'like', '%'.$request->input('aluEstado').'%');
    })
    ...
    if (count($cgts)) {
      ...
      $pdf->AliasNbPages();
      $pdf->SetTitle('Relacion de Inscritos');
      $pdf->SetFont('Times', '', 9);
      ...
      foreach ($cgts as $cgt) {
        ...
        $alumno = Curso::select('cursos.id as curso', 'cursos.curTipoIngreso', 'cursos.curTipoBeca',
          ...
          ->where('cursos.curTipoIngreso', "=", "PI");
        ...
        foreach ($alumno as $fila) {
          ...
          $datosExani = $fila->curExani ? $fila->curExani: "";

          $datoPagoInscripcion = Pago::where(["pagClaveAlu" => $fila->aluClave, "pagAnioPer" => $cgt->periodo->perAnioPago, "pagConcPago" => "99" ])->first();
          ...
          if ($GLOBALS['tipoReporte'] == "N") {
            ...
            $pdf->Cell($anchoExani, $altoCelda, $datosExani, $dibujarLinea, 0, 'C');
...
class PDF extends Fpdf
{
  function Header()
  {
    ...
    $this->Cell($anchoTituloIzquierdo, $altoTitulo, 'UNIVERSIDAD MODELO S.C.P.', $dibujarLinea, 0);
    ...
    $this->Ln($altoTitulo);
    $this->Cell(0, $altoTitulo, "InscritoPreinscritoController.php", $dibujarLinea, 0, 'R');
    $this->Ln($altoTitulo);
    ...
    if ($tipoReporte == "N") {
      ...
      $this->Cell($anchoExani, $altoCelda, $resExani, $dibujarLineaEncabezado, 0);
