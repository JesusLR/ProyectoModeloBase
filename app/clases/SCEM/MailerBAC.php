...
class MailerBAC
{
    ...
    */
    ...
    public function armar_correo() {
        ...
        $this->mail->SMTPDebug = 0; //3;                           // Enable verbose debug output
        $this->mail->isSMTP();                                // Set mailer to use SMTP
        $this->mail->Host = 'smtp.office365.com'; //'mail.unimodelo.com';             // Specify main and backup SMTP servers
        $this->mail->SMTPAuth = true;                         // Enable SMTP authentication
        $this->mail->Username = $this->username_email;        // SMTP username
        $this->mail->Password = $this->password_email;        // SMTP password
        $this->mail->SMTPSecure = 'tls'; //'ssl';                      // Enable TLS encryption, `ssl` also accepted
        $this->mail->Port = 587; // 465;                              // TCP port to connect to

        #Mail settings
        $this->mail->setFrom($this->username_email, 'Preparatoria Modelo');
