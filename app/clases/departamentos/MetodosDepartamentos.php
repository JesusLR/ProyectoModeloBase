...
class MetodosDepartamentos
{
    ...
    */
    public static function buscarSoloAcademicos($ubicacion_id = null, $depClaves = []): Collection
    {
        ...
        })->orderBy('depNivel', 'ASC')->get();
    }

    public static function buscarSoloAcademicosBAC($ubicacion_id = null, $depClaves = []): Collection
    {
        return Departamento::where(static function($query) use ($ubicacion_id, $depClaves) {
            if($ubicacion_id) {
                $query->where('ubicacion_id', $ubicacion_id);
    ...
    public static function buscarAEX($ubicacion_id = null, $depClaves = []): Collection
    {
        return Departamento::where(static function($query) use ($ubicacion_id, $depClaves) {
            if($ubicacion_id) {
                $query->where('ubicacion_id', $ubicacion_id);
    ...
    public static function buscarIDIOMAS($ubicacion_id = null, $depClaves = []): Collection
    {
        return Departamento::where(static function($query) use ($ubicacion_id, $depClaves) {
            if($ubicacion_id) {
                $query->where('ubicacion_id', $ubicacion_id);
