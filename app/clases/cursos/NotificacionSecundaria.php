...
class NotificacionSecundaria {
	...
	public function baja_realizada(Baja $baja)
	{
		$this->mail = new Mailer([
			'username_email' => 'bajas@modelo.edu.mx', // 'bajas@modelo.edu.mx', // bajas@unimodelo.com
		...
		if($this->ubicacion->ubiClave == 'CVA') {
			$director_campus = 'lcanche@modelo.edu.mx';
			$coordinador_secretaria_academica = 'mtuz@modelo.edu.mx';
		} else if($this->ubicacion->ubiClave == 'CME') {
			$director_campus = 'silviav.pool@modelo.edu.mx';
			$coordinador_secretaria_academica = 'ciraa.novelo@modelo.edu.mx';
			$this->mail->agregar_destinatario("lulu@modelo.edu.mx");
