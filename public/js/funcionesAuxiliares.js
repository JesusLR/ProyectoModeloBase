...
function apply_data_to_select(select_id, data_value, default_value = null) {
    ...
    let data = select.data(data_value) || default_value;
...
function getPlanesFederales(programa_id, targetSelect = 'plan_id', val = null, dataName = null) {
    var select = $('#' + targetSelect);
    var current_data = dataName || 'plan-id';
    var current_value = val || select.data(current_data);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
        url: base_url + '/api/planes_federales/' + programa_id,
        dataType: 'json',
        data: { programa_id: programa_id },
        success: function(planes) {
            if (planes) {
...
function getMaterias(plan_id, targetSelect = 'materia_id', val = null, dataName = null) {
    ...
    $.ajax({
        type: 'GET',
        url: base_url + '/api/getMateriasByPlan/' + plan_id,
        dataType: 'json',
        data: { plan_id: plan_id },
...
function getCgts_plan_periodo(plan_id, periodo_id, targetSelect = 'cgt_id', val = null, dataName = null) {
    var select = $('#' + targetSelect);
    let current_data = dataName || 'cgt-id';
    let current_value = val || select.data(current_data);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
...
function getCgtsPorSemestre(plan_id, periodo_id, semestre, targetSelect = 'cgt_id', val = null, dataName = 'cgt-id') {
    let select = $('#' + targetSelect);
    let current_value = val || select.data(dataName);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
        url: base_url + '/api/cgtsPorSemestre/' + plan_id + '/' + periodo_id + '/' + semestre,
        dataType: 'json',
        success: function(cgts) {
            if (cgts) {
                $.each(cgts, function(key, value) {
...
/** #################################################################
 *      FUNCIONES DE AJAX REQUEST PARA MÓDULOS DE TUTORIAS.
 * --------------------------------------------------------------- */


function getTutoriasPreguntas(requestParams, targetSelect = 'PreguntaID', val = null, dataName = null) {
    var select = $('#' + targetSelect);
    var current_data = dataName || 'pregunta-id';
    var current_value = val || select.data(current_data);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
        url: base_url + '/api/tutorias/preguntas',
        dataType: 'json',
        data: requestParams,
        success: function(preguntas) {
            if (preguntas) {
                $.each(preguntas, function(key, value) {
                    select.append(new Option(value.Nombre, value.PreguntaID));
                    (value.PreguntaID == current_value) && select.val(value.PreguntaID);
                });
                select.trigger('change');
                select.trigger('click');
            }
...
} //getTutoriasPreguntas.

function getTutoriasRespuestas(PreguntaID, targetSelect = 'RespuestaID', val = null, dataName = null) {
    var select = $('#' + targetSelect);
    var current_data = dataName || 'respuesta-id';
    var current_value = val || select.data(current_data);
    select.empty().append(new Option('SELECCIONE UNA OPCIÓN', ''));
    $.ajax({
        type: 'GET',
        url: base_url + '/api/tutorias/respuestas',
        dataType: 'json',
        data: { PreguntaID: PreguntaID },
        success: function(respuestas) {
            if (respuestas) {
                $.each(respuestas, function(key, value) {
                    select.append(new Option(value.Nombre, value.RespuestaID));
                    (value.PreguntaID == current_value) && select.val(value.PreguntaID);
                });
                select.trigger('change');
                select.trigger('click');
